...
public class PFChunkGenerator extends NoiseChunkGenerator<PFGenSettings>{
	...
	private static final float[] field_222567_h = Util.make(new float[25], (p_222575_0_) -> {
		for (int i = -2; i <= 2; i++) {
			for (int j = -2; j <= 2; j++) {
	...
	public PFChunkGenerator(IWorld world, BiomeProvider provider, PFGenSettings settings) {
	...
	protected double[] getBiomeNoiseColumn(int noiseX, int noiseZ) {
		...
		for (int k = -2; k <= 2; ++k) {
			for (int l = -2; l <= 2; ++l) {
				...
				float f6 = field_222567_h[k + 2 + (l + 2) * 5] / (f4 + 2.0F);
