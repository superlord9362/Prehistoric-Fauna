...
public class WaterloggedHorsetailsFeature extends Feature<JohnstoniaConfig> {
	...
	public boolean place(FeaturePlaceContext<JohnstoniaConfig> context) {
		...
		for (int j = 0; j < 15; ++j) {
			...
			if(world.getBlockState(blockpos.below()).getBlock() == Blocks.WATER && world.getBlockState(blockpos).getBlock() == Blocks.AIR && world.getBlockState(blockpos.above()).getBlock() == Blocks.AIR && (world.getBlockState(blockpos.below(2)).getBlock() == Blocks.COARSE_DIRT || world.getBlockState(blockpos.below(2)).getBlock() == PFBlocks.PACKED_LOAM.get() || world.getBlockState(blockpos.below(2)).getBlock() == PFBlocks.SILT.get() || world.getBlockState(blockpos.below(2)).getBlock() == Blocks.SAND || world.getBlockState(blockpos.below(2)).getBlock() == Blocks.DIRT || world.getBlockState(blockpos.below(2)).getBlock() == Blocks.MUD || world.getBlockState(blockpos.below(2)).getBlock() == Blocks.RED_SAND)) {
