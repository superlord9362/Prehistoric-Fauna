...
public class DjadochtaHutStructurePiece {

	private static final ResourceLocation djadochtaHut_Template = new ResourceLocation(PrehistoricFauna.MOD_ID, "djadochta_hut");

	private static final Map<ResourceLocation, BlockPos> structurePos = ImmutableMap.of(djadochtaHut_Template, BlockPos.ZERO);
	...
	public static void addStructure(StructureManager manager, BlockPos pos, Rotation rotation, StructurePieceAccessor accessor, Random random) {
		accessor.addPiece(new Piece(manager, djadochtaHut_Template, pos, rotation, 0));
	}

	public static class Piece extends TemplateStructurePiece {
		public Piece(StructureManager manager, ResourceLocation location, BlockPos pos, Rotation rotation, int position) {
			super(PFStructures.DJADOCHTA_HUT_PIECE, 0, manager, location, location.toString(), makeSettings(rotation), makePosition(location, pos, position));
		}

		public Piece(StructureManager manager, CompoundTag tag) {
			super(PFStructures.DJADOCHTA_HUT_PIECE, tag, manager, (position) -> {
