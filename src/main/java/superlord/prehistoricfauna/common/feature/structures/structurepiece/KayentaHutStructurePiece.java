...
public class KayentaHutStructurePiece {

	private static final ResourceLocation kayentaHut_Template = new ResourceLocation(PrehistoricFauna.MOD_ID, "kayenta_hut");

	private static final Map<ResourceLocation, BlockPos> structurePos = ImmutableMap.of(kayentaHut_Template, BlockPos.ZERO);
	...
	public static void addStructure(StructureManager manager, BlockPos pos, Rotation rotation, StructurePieceAccessor accessor, Random random) {
		accessor.addPiece(new Piece(manager, kayentaHut_Template, pos, rotation, 0));
	}

	public static class Piece extends TemplateStructurePiece {
		public Piece(StructureManager manager, ResourceLocation location, BlockPos pos, Rotation rotation, int position) {
			super(PFStructures.KAYENTA_HUT_PIECE, 0, manager, location, location.toString(), makeSettings(rotation), makePosition(location, pos, position));
		}

		public Piece(StructureManager manager, CompoundTag tag) {
			super(PFStructures.KAYENTA_HUT_PIECE, tag, manager, (position) -> {
