...
public class PortalChamberStructurePiece {

	private static final ResourceLocation portalChamber_Template = new ResourceLocation(PrehistoricFauna.MOD_ID, "portal_chamber");

	private static final Map<ResourceLocation, BlockPos> structurePos = ImmutableMap.of(portalChamber_Template, BlockPos.ZERO);
	...
	public static void addStructure(StructureManager manager, BlockPos pos, Rotation rotation, StructurePieceAccessor accessor, Random random) {
		accessor.addPiece(new Piece(manager, portalChamber_Template, pos, rotation, 0));
	}

	public static class Piece extends TemplateStructurePiece {
		public Piece(StructureManager manager, ResourceLocation location, BlockPos pos, Rotation rotation, int position) {
			super(PFStructures.PORTAL_CHAMBER_PIECE, 0, manager, location, location.toString(), makeSettings(rotation), makePosition(location, pos, position));
		}

		public Piece(StructureManager manager, CompoundTag tag) {
			super(PFStructures.PORTAL_CHAMBER_PIECE, tag, manager, (position) -> {
