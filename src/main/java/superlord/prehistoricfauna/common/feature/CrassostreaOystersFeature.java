...
public class CrassostreaOystersFeature extends Feature<CrassostreaOystersConfig> {
	...
	public boolean place(FeaturePlaceContext<CrassostreaOystersConfig> context) {
		...
		for(int j = 0; j < 24; ++j) {
			...
			if (worldIn.getBlockState(blockpos).getBlock() == Blocks.WATER && worldIn.getBlockState(blockpos.below()).getBlock() == Blocks.SAND || worldIn.getBlockState(blockpos).getBlock() == Blocks.WATER && worldIn.getBlockState(blockpos.below()).getBlock() == Blocks.DIRT || worldIn.getBlockState(blockpos).getBlock() == Blocks.WATER && worldIn.getBlockState(blockpos.below()).getBlock() == Blocks.COARSE_DIRT || worldIn.getBlockState(blockpos).getBlock() == Blocks.WATER && worldIn.getBlockState(blockpos.below()).getBlock() == PFBlocks.PACKED_LOAM.get() || worldIn.getBlockState(blockpos).getBlock() == Blocks.WATER && worldIn.getBlockState(blockpos.below()).getBlock() == Blocks.STONE) {
