...
public class Taxus7 extends PFAbstractTreeFeature<PFTreeConfig> {

	public Taxus7(Codec<PFTreeConfig> configIn) {
		super(configIn);
	}
	...
	protected boolean generate(Set<BlockPos> changedBlocks, WorldGenLevel world, Random rand, BlockPos pos, BoundingBox boundsIn, boolean isSapling, PFTreeConfig config) {
		...
		if (pos.getY() + randTreeHeight + 1 < world.getHeight()) {
			if (!this.isAnotherTreeNearby(world, pos, randTreeHeight, 0, isSapling)) {
				...
				//buildTrunkBase(pos, changedBlocks, world, config, rand, boundsIn, mainmutable.set(pos).move(-1, 0, 0).immutable(), mainmutable.set(pos).move(-1, 0, 1).immutable(), mainmutable.set(pos).move(0, 0, 0).immutable(), mainmutable.set(pos).move(0, 0, 1).immutable());
				...
				placeBranch(pos, config, rand, changedBlocks, world, mainmutable.set(pos).move(-3, randTreeHeight - 10, 1), boundsIn);
