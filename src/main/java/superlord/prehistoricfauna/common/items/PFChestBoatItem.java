...
public class PFChestBoatItem extends Item {
	...
	public PFChestBoatItem(PFBoatTypes type, Item.Properties properties) {
		super(properties);
		this.type = type;
	}
	   ...
	   public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand) {
		...
		if (raytraceresult.getType() == HitResult.Type.MISS) {
			...
			if (raytraceresult.getType() == HitResult.Type.BLOCK) {
				PFChestBoat PFChestBoat = new PFChestBoat(world, raytraceresult.getLocation().x, raytraceresult.getLocation().y, raytraceresult.getLocation().z);
				PrehistoricFauna.LOGGER.info("BOAT ENTITY: " + PFChestBoat.getEntityData());
				PFChestBoat.setPFChestBoatType(this.type);
				PFChestBoat.yRotO = player.yRotO;
				if (!world.noCollision(PFChestBoat, PFChestBoat.getBoundingBox().inflate(-0.1D))) {
					return InteractionResultHolder.fail(itemstack);
				} else {
					if (!world.isClientSide) {
						world.addFreshEntity(PFChestBoat);
