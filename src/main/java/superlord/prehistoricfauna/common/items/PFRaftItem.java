...
public class PFRaftItem extends Item {

	private static final Predicate<Entity> RIDERS = EntitySelector.NO_SPECTATORS.and(Entity::canBeCollidedWith);
	private final PFRaftTypes type;

	public PFRaftItem(PFRaftTypes type, Item.Properties properties) {
		super(properties);
		this.type = type;
	}
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand) {
		...
		if (raytraceresult.getType() == HitResult.Type.MISS) {
			...
			if (raytraceresult.getType() == HitResult.Type.BLOCK) {
				PFRaft PFRaft = new PFRaft(world, raytraceresult.getLocation().x, raytraceresult.getLocation().y, raytraceresult.getLocation().z);
				PrehistoricFauna.LOGGER.info("BOAT ENTITY: " + PFRaft.getEntityData());
				PFRaft.setPFRaftType(this.type);
				PFRaft.yRotO = player.yRotO;
				if (!world.noCollision(PFRaft, PFRaft.getBoundingBox().inflate(-0.1D))) {
					return InteractionResultHolder.fail(itemstack);
				} else {
					if (!world.isClientSide) {
						world.addFreshEntity(PFRaft);
