...
public class CarvedTuberBlock extends HorizontalBlock implements IArmorVanishable {
	...
	private static final Predicate<BlockState> IS_TUBER = (state) -> {
		return state != null && (state.isIn(PFBlocks.CARVED_TUBER) || state.isIn(PFBlocks.CARVED_TUBER_LIT) || state.isIn(PFBlocks.CARVED_TUBER_SOUL_LIT) || state.isIn(PFBlocks.CARVED_TUBER_TIME_LIT));
	};
	...
	private BlockPattern getSnowmanPattern() {
		if (this.snowmanPattern == null) {
			this.snowmanPattern = BlockPatternBuilder.start().aisle("^", "#", "#").where('^', CachedBlockInfo.hasState(IS_TUBER)).where('#', CachedBlockInfo.hasState(BlockStateMatcher.forBlock(Blocks.SNOW_BLOCK))).build();
		}
	...
	private BlockPattern getGolemPattern() {
		if (this.golemPattern == null) {
			this.golemPattern = BlockPatternBuilder.start().aisle("~^~", "###", "~#~").where('^', CachedBlockInfo.hasState(IS_TUBER)).where('#', CachedBlockInfo.hasState(BlockStateMatcher.forBlock(Blocks.IRON_BLOCK))).where('~', CachedBlockInfo.hasState(BlockMaterialMatcher.forMaterial(Material.AIR))).build();
		}
