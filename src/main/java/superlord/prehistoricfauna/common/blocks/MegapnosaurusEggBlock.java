...
public class MegapnosaurusEggBlock extends Block {
	...
	public MegapnosaurusEggBlock(Block.Properties properties) {
		super(properties);
		...
		if (this.canGrow(worldIn, state) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					MegapnosaurusEntity megapnosaurusentity = PFEntities.MEGAPNOSAURUS_ENTITY.create(worldIn);
					megapnosaurusentity.setGrowingAge(-24000);
					megapnosaurusentity.setLocationAndAngles((double) pos.getX() + 0.3D + (double) j * 0.2D,
							(double) pos.getY(), (double) pos.getZ() + 0.3D, 0.0F, 0.0F);
					worldIn.addEntity(megapnosaurusentity);
				}
			}
		}
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		if (trampler instanceof MegapnosaurusEntity || trampler instanceof CitipatiEntity || trampler instanceof TelmasaurusEntity) {
			return false;
		} else {
