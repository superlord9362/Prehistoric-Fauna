...
public class CitipatiEggBlock extends Block {
	...
	public CitipatiEggBlock(Block.Properties properties) {
		super(properties);
		...
		if (this.canGrow(worldIn) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for(int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					CitipatiEntity citipatientity = PFEntities.CITIPATI_ENTITY.create(worldIn);
					citipatientity.setGrowingAge(-24000);
					citipatientity.setLocationAndAngles((double)pos.getX() + 0.3D + (double)j * 0.2D, (double)pos.getY(), (double)pos.getZ() + 0.3D, 0.0F, 0.0F);
					worldIn.addEntity(citipatientity);
				}
			}
		}
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		if (trampler instanceof CitipatiEntity) {
			return false;
		} else {
