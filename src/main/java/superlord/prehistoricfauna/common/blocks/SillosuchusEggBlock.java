...
public class SillosuchusEggBlock extends Block {
		...
		if (this.canGrow(worldIn, state) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					SillosuchusEntity sillosuchusentity = PFEntities.SILLOSUCHUS_ENTITY.create(worldIn);
					sillosuchusentity.setGrowingAge(-24000);
					sillosuchusentity.setLocationAndAngles((double) pos.getX() + 0.3D + (double) j * 0.2D,
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		if (trampler instanceof SillosuchusEntity || trampler instanceof CitipatiEntity || trampler instanceof TelmasaurusEntity) {
			return false;
		} else {
