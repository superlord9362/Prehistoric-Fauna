...
public class ThescelosaurusEggBlock extends Block {
	...
	public ThescelosaurusEggBlock(Block.Properties properties) {
		super(properties);
		...
		if (this.canGrow(worldIn, state) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					ThescelosaurusEntity thescelosaurusentity = PFEntities.THESCELOSAURUS_ENTITY.create(worldIn);
					thescelosaurusentity.setGrowingAge(-24000);
					thescelosaurusentity.setLocationAndAngles((double) pos.getX() + 0.3D + (double) j * 0.2D,
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		if (trampler instanceof ThescelosaurusEntity || trampler instanceof CitipatiEntity || trampler instanceof TelmasaurusEntity) {
			return false;
		} else {
