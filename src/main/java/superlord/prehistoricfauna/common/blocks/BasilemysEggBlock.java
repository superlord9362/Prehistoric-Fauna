...
public class BasilemysEggBlock extends Block {
		...
		if (this.canGrow(worldIn, state) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					BasilemysEntity basilemysentity = PFEntities.BASILEMYS_ENTITY.create(worldIn);
					basilemysentity.setGrowingAge(-24000);
					basilemysentity.setLocationAndAngles((double) pos.getX() + 0.3D + (double) j * 0.2D,
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		if (trampler instanceof BasilemysEntity || trampler instanceof CitipatiEntity || trampler instanceof TelmasaurusEntity) {
			return false;
		} else {
