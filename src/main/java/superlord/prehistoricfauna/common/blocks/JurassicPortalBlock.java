...
public class JurassicPortalBlock extends BreakableBlock {
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState facingState, IWorld world, BlockPos currentPos, BlockPos facingPos) {
		...
		return !flag && facingState.getBlock() != this && !(new JurassicPortalBlock.Size(world, currentPos, directionAxis1)).canCreatePortal() ? Blocks.AIR.getDefaultState() : super.updatePostPlacement(state, facing, facingState, world, currentPos, facingPos);
	}
		...
		if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() ) {
			if (entity.func_242280_ah() ) {
				...
				if (entity.world instanceof ServerWorld) {
					...
					RegistryKey<World> registryKey = entity.world.getDimensionKey() == PFDimensions.JURASSIC_WORLD ? World.OVERWORLD : PFDimensions.JURASSIC_WORLD;
	...
	public static class Size {
		...
		private boolean isEmptyBlock(BlockState state) {
			...
			return state.isAir() || block == PFBlocks.JURASSIC_TIME_BLOCK || block == PORTAL;
		}
		...
		public void placePortalBlocks() {
			BlockState state = PORTAL.getDefaultState().with(JurassicPortalBlock.AXIS, this.axis);
