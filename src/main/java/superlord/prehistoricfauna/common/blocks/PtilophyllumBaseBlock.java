...
public class PtilophyllumBaseBlock extends Block {
	...
	public PtilophyllumBaseBlock(PtilophyllumBlock plantBlock, AbstractBlock.Properties builder) {
		super(builder);
	...
	public boolean isValidPosition(BlockState state, IWorldReader worldIn, BlockPos pos) {
		BlockState blockstate = worldIn.getBlockState(pos.down());
		if (blockstate.getBlock() != this.plantBlock && !blockstate.isIn(Tags.Blocks.DIRT)) {
	...
	private static void growTreeRecursive(IWorld worldIn, BlockPos branchPos, Random rand, BlockPos originalBranchPos, int maxHorizontalDistance, int iterations) {
		PtilophyllumBlock chorusplantblock = (PtilophyllumBlock)PFBlocks.PTILOPHYLLUM_WOOD;
		...
		if (!flag) {
			worldIn.setBlockState(branchPos.up(i), PFBlocks.PTILOPHYLLUM_BASE.getDefaultState().with(AGE, Integer.valueOf(5)), 2);
		}
