		...
		if (this.canGrow(worldIn) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					CeratosaurusEntity ceratosaurusentity = PFEntities.CERATOSAURUS_ENTITY.create(worldIn);
					ceratosaurusentity.setGrowingAge(-24000);
					ceratosaurusentity.setLocationAndAngles((double) pos.getX() + 0.3D + (double) j * 0.2D, (double) pos.getY(), (double) pos.getZ() + 0.3D, 0.0F, 0.0F);
