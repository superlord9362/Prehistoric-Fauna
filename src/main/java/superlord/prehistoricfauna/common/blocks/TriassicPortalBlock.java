...
import superlord.prehistoricfauna.world.dimension.TriassicTeleporter;
...
public class TriassicPortalBlock extends BreakableBlock {
	...
	public TriassicPortalBlock(Properties properties) {
		super(properties);
	...
	public boolean tryToCreatePortal(World world, BlockPos pos) {
		TriassicPortalBlock.Size triassicPortalSize = this.isPortal(world, pos);
		if (triassicPortalSize != null && this.canCreatePortalByWorld(world, pos)) {
			triassicPortalSize.placePortalBlocks();
			return true;
		} else {
			return false;
		}
	...
	public TriassicPortalBlock.Size isPortal(IWorld world, BlockPos pos) {
		TriassicPortalBlock.Size triassicPortalSizeX = new TriassicPortalBlock.Size(world, pos, Direction.Axis.X);
		if (triassicPortalSizeX.isValid() && triassicPortalSizeX.portalBlockCount == 0) {
			return triassicPortalSizeX;
		} else {
			TriassicPortalBlock.Size triassicPortalSizeZ = new TriassicPortalBlock.Size(world, pos, Direction.Axis.Z);
			return triassicPortalSizeZ.isValid() && triassicPortalSizeZ.portalBlockCount == 0 ? triassicPortalSizeZ : null;
		}
	}
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState facingState, IWorld world, BlockPos currentPos, BlockPos facingPos) {
		...
		return !flag && facingState.getBlock() != this && !(new TriassicPortalBlock.Size(world, currentPos, directionAxis1)).canCreatePortal() ? Blocks.AIR.getDefaultState() : super.updatePostPlacement(state, facing, facingState, world, currentPos, facingPos);
	}
		...
		if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() ) {
			if (entity.func_242280_ah() ) {
				...
				if (entity.world instanceof ServerWorld) {
					...
					RegistryKey<World> registryKey = entity.world.getDimensionKey() == PFDimensions.TRIASSIC_WORLD ? World.OVERWORLD : PFDimensions.TRIASSIC_WORLD;
					ServerWorld serverWorld1 = minecraftServer.getWorld(registryKey);
					if (serverWorld1 != null && !entity.isPassenger()) {
						entity.func_242279_ag();
						entity.changeDimension(serverWorld1, new TriassicTeleporter(serverWorld1));
					}
				}
			}
		}
	...
	public static class Size {
		...
		private final Block PORTAL = PFBlocks.TRIASSIC_PORTAL.get();
		...
		private boolean isEmptyBlock(BlockState state) {
			...
			return state.isAir() || block == PFBlocks.TRIASSIC_TIME_BLOCK || block == PORTAL;
		}
		...
		public void placePortalBlocks() {
			BlockState state = PORTAL.getDefaultState().with(TriassicPortalBlock.AXIS, this.axis);
