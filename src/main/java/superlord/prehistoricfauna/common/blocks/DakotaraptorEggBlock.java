...
public class DakotaraptorEggBlock extends Block {
		...
		if (this.canGrow(worldIn, state) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					DakotaraptorEntity dakotaraptorentity = PFEntities.DAKOTARAPTOR_ENTITY.create(worldIn);
					dakotaraptorentity.setGrowingAge(-24000);
					dakotaraptorentity.setLocationAndAngles((double) pos.getX() + 0.3D + (double) j * 0.2D,
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		if (trampler instanceof DakotaraptorEntity || trampler instanceof CitipatiEntity || trampler instanceof TelmasaurusEntity) {
			return false;
		} else {
