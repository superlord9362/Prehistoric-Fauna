...
public class CamarasaurusEggBlock extends Block {
		...
		if (this.canGrow(worldIn, state) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					CamarasaurusEntity camarasaurusentity = PFEntities.CAMARASAURUS_ENTITY.create(worldIn);
					camarasaurusentity.setGrowingAge(-24000);
					camarasaurusentity.setLocationAndAngles((double) pos.getX() + 0.3D + (double) j * 0.2D,
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		if (trampler instanceof CamarasaurusEntity || trampler instanceof CitipatiEntity || trampler instanceof TelmasaurusEntity) {
			return false;
		} else {
