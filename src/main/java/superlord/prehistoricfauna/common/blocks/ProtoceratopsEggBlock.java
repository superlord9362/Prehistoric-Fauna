...
public class ProtoceratopsEggBlock extends Block {
      ...
      if (this.canGrow(worldIn) && this.hasProperHabitat(worldIn, pos)) {
         ...
         if (i < 2) {
            ...
            for(int j = 0; j < state.get(EGGS); ++j) {
               ...
               protoceratopsEntity.setLocationAndAngles((double)pos.getX() + 0.3D + (double)j * 0.2D, (double)pos.getY(), (double)pos.getZ() + 0.3D, 0.0F, 0.0F);
   ...
   private boolean canTrample(World worldIn, Entity trampler) {
      if (trampler instanceof ProtoceratopsEntity) {
         return false;
      } else {
