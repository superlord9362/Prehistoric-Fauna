...
public class ProtoceratopsEggBlock extends Block {
	...
	public ProtoceratopsEggBlock(Block.Properties properties) {
		super(properties);
		...
		if (this.canGrow(worldIn, state) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					ProtoceratopsEntity protoceratopsentity = PFEntities.PROTOCERATOPS_ENTITY.create(worldIn);
					protoceratopsentity.setGrowingAge(-24000);
					protoceratopsentity.setLocationAndAngles((double) pos.getX() + 0.3D + (double) j * 0.2D,
							(double) pos.getY(), (double) pos.getZ() + 0.3D, 0.0F, 0.0F);
					worldIn.addEntity(protoceratopsentity);
				}
			}
		}
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		if (trampler instanceof ProtoceratopsEntity || trampler instanceof CitipatiEntity || trampler instanceof TelmasaurusEntity) {
			return false;
		} else {
