...
public class MegapnosaurusEntity extends DinosaurEntity {

	private static final DataParameter<Boolean> HAS_EGG = EntityDataManager.createKey(MegapnosaurusEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> IS_DIGGING = EntityDataManager.createKey(MegapnosaurusEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> ALBINO = EntityDataManager.createKey(MegapnosaurusEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> MELANISTIC = EntityDataManager.createKey(MegapnosaurusEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> NATURAL_LOVE = EntityDataManager.createKey(MegapnosaurusEntity.class, DataSerializers.BOOLEAN);
	...
	public MegapnosaurusEntity(EntityType<? extends MegapnosaurusEntity> type, World worldIn) {
		super(type, worldIn);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new MegapnosaurusEntity.MeleeAttackGoal());
		...
		this.targetSelector.addGoal(1, new MegapnosaurusEntity.HurtByTargetGoal());
		this.targetSelector.addGoal(2, new MegapnosaurusEntity.AttackPlayerGoal());
		this.goalSelector.addGoal(0, new MegapnosaurusEntity.LayEggGoal(this, 1.0D));
		this.goalSelector.addGoal(0, new MegapnosaurusEntity.MateGoal(this, 1.0D));
		this.goalSelector.addGoal(0, new MegapnosaurusEntity.NaturalMateGoal(this, 1.0D));
		...
		this.targetSelector.addGoal(0, new MegapnosaurusEntity.CarnivoreHuntGoal(this, LivingEntity.class, 10, true, false, (p_213487_1_) -> {
	...
	public void livingTick() {
		...
		if (!this.isAIDisabled()) {
			List<MegapnosaurusEntity> list = this.world.getEntitiesWithinAABB(this.getClass(), this.getBoundingBox().grow(20.0D, 20.0D, 20.0D));
	...
	public static AttributeModifierMap.MutableAttribute createAttributes() {
		return MobEntity.func_233666_p_().createMutableAttribute(Attributes.MAX_HEALTH, 6.0D).createMutableAttribute(Attributes.MOVEMENT_SPEED, 0.25D).createMutableAttribute(Attributes.FOLLOW_RANGE, 20.0D).createMutableAttribute(Attributes.ATTACK_DAMAGE, 2.0D);
	}
	...
	protected SoundEvent getAmbientSound() {
		return this.isAsleep() ? null : SoundInit.MEGAPNOSAURUS_IDLE;
	}

	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return SoundInit.MEGAPNOSAURUS_HURT;
	}

	protected SoundEvent getDeathSound() {
		return SoundInit.MEGAPNOSAURUS_DEATH;
	}

	protected void playWarningSound() {
		if (this.warningSoundTicks <= 0) {
			this.playSound(SoundInit.MEGAPNOSAURUS_WARN, 1.0F, this.getSoundPitch());
	...
	class AttackPlayerGoal extends NearestAttackableTargetGoal<PlayerEntity> {
		public AttackPlayerGoal() {
			super(MegapnosaurusEntity.this, PlayerEntity.class, 20, true, true, (Predicate<LivingEntity>)null);
		}

		public boolean shouldExecute() {
			if (MegapnosaurusEntity.this.isChild()) {
				return false;
			} else {
				if (super.shouldExecute()) {
					for(MegapnosaurusEntity megapnosaurus : MegapnosaurusEntity.this.world.getEntitiesWithinAABB(MegapnosaurusEntity.class, MegapnosaurusEntity.this.getBoundingBox().grow(8.0D, 4.0D, 8.0D))) {
	...
	class HurtByTargetGoal extends net.minecraft.entity.ai.goal.HurtByTargetGoal {
		public HurtByTargetGoal() {
			super(MegapnosaurusEntity.this);
		}
	...
	class MeleeAttackGoal extends net.minecraft.entity.ai.goal.MeleeAttackGoal {
		public MeleeAttackGoal() {
			super(MegapnosaurusEntity.this, 1.25D, true);
		}
		...
		protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
			...
			if (distToEnemySqr <= d0 && this.func_234040_h_()) {
				...
				if (this.func_234041_j_() <= 10) {
					MegapnosaurusEntity.this.playWarningSound();
				}
			} else {
				this.func_234039_g_();
			}
	...
	static class LayEggGoal extends MoveToBlockGoal {
		private final MegapnosaurusEntity megapnosaurus;

		LayEggGoal(MegapnosaurusEntity megapnosaurus, double speedIn) {
			super(megapnosaurus, speedIn, 16);
			this.megapnosaurus = megapnosaurus;
		}

		public boolean shouldExecute() {
			return this.megapnosaurus.hasEgg() ? super.shouldExecute() : false;
		}

		public boolean shouldContinueExecuting() {
			return super.shouldContinueExecuting() && this.megapnosaurus.hasEgg();
		}

		public void tick() {
			super.tick();
			BlockPos blockpos = new BlockPos(this.megapnosaurus.getPositionVec());
			if (!this.megapnosaurus.isInWater() && this.getIsAboveDestination()) {
				if (this.megapnosaurus.isDigging < 1) {
					this.megapnosaurus.setDigging(true);
				} else if (this.megapnosaurus.isDigging > 200) {
					World world = this.megapnosaurus.world;
					world.playSound((PlayerEntity)null, blockpos, SoundEvents.ENTITY_TURTLE_LAY_EGG, SoundCategory.BLOCKS, 0.3F, 0.9F + world.rand.nextFloat() * 0.2F);
					world.setBlockState(this.destinationBlock.up(), PFBlocks.MEGAPNOSAURUS_EGG.getDefaultState().with(DinosaurEggBlock.EGGS, Integer.valueOf(this.megapnosaurus.rand.nextInt(4) + 1)), 3);
					this.megapnosaurus.setHasEgg(false);
					this.megapnosaurus.setDigging(false);
					this.megapnosaurus.setInLove(600);
				}
	...
	static class MateGoal extends BreedGoal {
		...
		MateGoal(MegapnosaurusEntity megapnosaurus, double speedIn) {
			super(megapnosaurus, speedIn);
			this.megapnosaurus = megapnosaurus;
		}

		public boolean shouldExecute() {
			return super.shouldExecute() && !this.megapnosaurus.hasEgg() && !this.megapnosaurus.isInLoveNaturally();
		}
	...
	static class NaturalMateGoal extends BreedGoal {
		...
		NaturalMateGoal(MegapnosaurusEntity megapnosaurus, double speed) {
			super(megapnosaurus, speed);
			this.megapnosaurus = megapnosaurus;
		}

		public boolean shouldExecute() {
			return super.shouldExecute() && !this.megapnosaurus.hasEgg() && this.megapnosaurus.getCurrentHunger() >= this.megapnosaurus.getThreeQuartersHunger() && this.megapnosaurus.ticksExisted % 60 == 0 && (PrehistoricFaunaConfig.naturalEggBlockLaying || PrehistoricFaunaConfig.naturalEggItemLaying) && this.megapnosaurus.isInLoveNaturally();
		}

		protected void spawnBaby() {
			if (PrehistoricFaunaConfig.naturalEggItemLaying) {
				this.megapnosaurus.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.megapnosaurus.rand.nextFloat() - this.megapnosaurus.rand.nextFloat()) * 0.2F + 1.0F);
				int eggAmount = this.megapnosaurus.rand.nextInt(4);
				if (eggAmount == 0) {
					this.megapnosaurus.entityDropItem(PFBlocks.MEGAPNOSAURUS_EGG.asItem());
				}
				if (eggAmount == 1) {
					this.megapnosaurus.entityDropItem(PFBlocks.MEGAPNOSAURUS_EGG.asItem());
					this.megapnosaurus.entityDropItem(PFBlocks.MEGAPNOSAURUS_EGG.asItem());
	...
	public AgeableEntity func_241840_a(ServerWorld p_241840_1_, AgeableEntity p_241840_2_) {
		MegapnosaurusEntity entity = new MegapnosaurusEntity(PFEntities.MEGAPNOSAURUS_ENTITY, this.world);
	...
	public class CarnivoreHuntGoal extends NearestAttackableTargetGoal {
		...
		public boolean shouldExecute() {
			return super.shouldExecute() && MegapnosaurusEntity.this.getCurrentHunger() <= MegapnosaurusEntity.this.getHalfHunger() && !MegapnosaurusEntity.this.isChild() && PrehistoricFaunaConfig.advancedHunger == true;
		}

		public boolean shouldContinueExecuting() {
			return (MegapnosaurusEntity.this.getCurrentHunger() < MegapnosaurusEntity.this.maxHunger) || (super.shouldContinueExecuting()  && (target.getHealth() <= (target.getMaxHealth() / 2) && PrehistoricFaunaConfig.advancedHunger == true || target.getHealth() <= MegapnosaurusEntity.this.getHealth())) && PrehistoricFaunaConfig.advancedHunger == true;
		}

		public void tick() {
			LivingEntity target = MegapnosaurusEntity.this.getAttackTarget();
			if (target instanceof RabbitEntity) {
				if (target.getHealth() == 0) {
					if (MegapnosaurusEntity.this.getCurrentHunger() + 3 >= MegapnosaurusEntity.this.maxHunger) {
						MegapnosaurusEntity.this.setHunger(MegapnosaurusEntity.this.maxHunger);
					} else {
						MegapnosaurusEntity.this.setHunger(currentHunger + 3);
					}
				}
			}
			if (target instanceof DidelphodonEntity || target instanceof EilenodonEntity || target instanceof HesperornithoidesEntity || target instanceof HyperodapedonEntity || target instanceof ChickenEntity || target instanceof ScutellosaurusEntity) {
				if (target.getHealth() == 0) {
					if (MegapnosaurusEntity.this.getCurrentHunger() + 4 >= MegapnosaurusEntity.this.maxHunger) {
						MegapnosaurusEntity.this.setHunger(MegapnosaurusEntity.this.maxHunger);
					} else {
						MegapnosaurusEntity.this.setHunger(currentHunger + 4);
					}
				}
			}
			if (target instanceof ChromogisaurusEntity || target instanceof TelmasaurusEntity || target instanceof ParrotEntity || target instanceof KayentatheriumEntity) {
				if (target.getHealth() == 0) {
					if (MegapnosaurusEntity.this.getCurrentHunger() + 6 >= MegapnosaurusEntity.this.maxHunger) {
						MegapnosaurusEntity.this.setHunger(MegapnosaurusEntity.this.maxHunger);
					} else {
						MegapnosaurusEntity.this.setHunger(currentHunger + 6);
					}
				}
			}
			if (target instanceof BasilemysEntity || target instanceof ExaeretodonEntity || target instanceof CatEntity || target instanceof FoxEntity || target instanceof CowEntity || target instanceof MooshroomEntity || target instanceof PigEntity || target instanceof OcelotEntity || target instanceof AepyornithomimusEntity || target instanceof ProtoceratopsEntity) {
				if (target.getHealth() == 0) {
					if (MegapnosaurusEntity.this.getCurrentHunger() + 10 >= MegapnosaurusEntity.this.maxHunger) {
						MegapnosaurusEntity.this.setHunger(MegapnosaurusEntity.this.maxHunger);
					} else {
						MegapnosaurusEntity.this.setHunger(currentHunger + 10);
					}
				}
			}
			if (target instanceof DryosaurusEntity || target instanceof ThescelosaurusEntity || target instanceof SarahsaurusEntity || target instanceof ScelidosaurusEntity) {
				if (target.getHealth() == 0) {
					if (MegapnosaurusEntity.this.getCurrentHunger() + 15 >= MegapnosaurusEntity.this.maxHunger) {
						MegapnosaurusEntity.this.setHunger(MegapnosaurusEntity.this.maxHunger);
					} else {
						MegapnosaurusEntity.this.setHunger(currentHunger + 15);
					}
				}
			}
			if (target instanceof PlayerEntity) {
				if (target.getHealth() == 0) {
					if (MegapnosaurusEntity.this.getCurrentHunger() + 20 >= MegapnosaurusEntity.this.maxHunger) {
						MegapnosaurusEntity.this.setHunger(MegapnosaurusEntity.this.maxHunger);
					} else {
						MegapnosaurusEntity.this.setHunger(currentHunger + 20);
					}
				}
			}
			super.tick();
	...
	public class BabyCarnivoreHuntGoal extends NearestAttackableTargetGoal {
		...
		public boolean shouldContinueExecuting() {
			return MegapnosaurusEntity.this.getCurrentHunger() < MegapnosaurusEntity.this.maxHunger && PrehistoricFaunaConfig.advancedHunger == true || !MegapnosaurusEntity.this.isChild() && PrehistoricFaunaConfig.advancedHunger == true;
		}
