...
public class CeratosaurusEntity extends DinosaurEntity {
	...
	public class CarnivoreHuntGoal extends NearestAttackableTargetGoal {
		...
		public void tick() {
			...
			if (target instanceof VelociraptorEntity || target instanceof BasilemysEntity || target instanceof ChromogisaurusEntity || target instanceof ExaeretodonEntity || target instanceof TelmasaurusEntity || target instanceof MegapnosaurusEntity || target instanceof KayentatheriumEntity) {
			...
			if (target instanceof AepyornithomimusEntity || target instanceof WolfEntity || target instanceof SheepEntity || target instanceof ProtoceratopsEntity) {
				if (target.getHealth() == 0) {
			...
			if (target instanceof CatEntity || target instanceof FoxEntity || target instanceof CowEntity || target instanceof MooshroomEntity || target instanceof PigEntity || target instanceof OcelotEntity || target instanceof DryosaurusEntity || target instanceof ThescelosaurusEntity || target instanceof SarahsaurusEntity || target instanceof ScelidosaurusEntity) {
				if (target.getHealth() == 0) {
