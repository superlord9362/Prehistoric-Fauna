...
public class ThescelosaurusEntity extends DinosaurEntity {
	...
	private static final DataParameter<Boolean> EATING = EntityDataManager.createKey(ThescelosaurusEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> NATURAL_LOVE = EntityDataManager.createKey(ThescelosaurusEntity.class, DataSerializers.BOOLEAN);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(0, new ThescelosaurusEntity.NaturalMateGoal(this, 1.0D));
		...
		this.goalSelector.addGoal(0, new ThescelosaurusEntity.HerbivoreEatGoal((double)1.2F, 12, 2));
	...
	public void livingTick() {
		...
		if (!this.isAIDisabled()) {
			List<ThescelosaurusEntity> list = this.world.getEntitiesWithinAABB(this.getClass(), this.getBoundingBox().grow(20.0D, 20.0D, 20.0D));
	...
	public static AttributeModifierMap.MutableAttribute createAttributes() {
		return MobEntity.func_233666_p_().createMutableAttribute(Attributes.MAX_HEALTH, 15.0D).createMutableAttribute(Attributes.MOVEMENT_SPEED, 0.23D);
	}
		...
		public void tick() {
			...
			if (!this.thescelosaurus.isInWater() && this.getIsAboveDestination()) {
				if (this.thescelosaurus.isDigging < 1) {
					...
					world.setBlockState(this.destinationBlock.up(), PFBlocks.THESCELOSAURUS_EGG.getDefaultState().with(DinosaurEggBlock.EGGS, Integer.valueOf(this.thescelosaurus.rand.nextInt(4) + 1)), 3);
	...
	static class MateGoal extends BreedGoal {
		...
		public boolean shouldExecute() {
			return super.shouldExecute() && !this.thescelosaurus.hasEgg() && !this.thescelosaurus.isInLoveNaturally();
		}
	...
	static class NaturalMateGoal extends BreedGoal {
		...
		public boolean shouldExecute() {
			return super.shouldExecute() && !this.thescelosaurus.hasEgg() && this.thescelosaurus.getCurrentHunger() >= this.thescelosaurus.getThreeQuartersHunger() && this.thescelosaurus.ticksExisted % 60 == 0 && (PrehistoricFaunaConfig.naturalEggBlockLaying || PrehistoricFaunaConfig.naturalEggItemLaying) && this.thescelosaurus.isInLoveNaturally();
		}

		protected void spawnBaby() {
			if (PrehistoricFaunaConfig.naturalEggItemLaying) {
				this.thescelosaurus.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.thescelosaurus.rand.nextFloat() - this.thescelosaurus.rand.nextFloat()) * 0.2F + 1.0F);
				int eggAmount = this.thescelosaurus.rand.nextInt(4);
				if (eggAmount == 0) {
					this.thescelosaurus.entityDropItem(PFBlocks.THESCELOSAURUS_EGG.asItem());
				}
				if (eggAmount == 1) {
					this.thescelosaurus.entityDropItem(PFBlocks.THESCELOSAURUS_EGG.asItem());
					this.thescelosaurus.entityDropItem(PFBlocks.THESCELOSAURUS_EGG.asItem());
	...
	public class HerbivoreEatGoal extends MoveToBlockGoal {
		...
		public HerbivoreEatGoal(double p_i50737_2_, int p_i50737_4_, int p_i50737_5_) {
			super(ThescelosaurusEntity.this, p_i50737_2_, p_i50737_4_, p_i50737_5_);
		}
		...
		public void tick() {
			if (this.getIsAboveDestination()) {
				...
				if (this.field_220731_g % 5 == 1) {
					ThescelosaurusEntity.this.world.playSound((PlayerEntity)null, this.destinationBlock, SoundEvents.BLOCK_GRASS_HIT, SoundCategory.NEUTRAL, 1, 1);
				}
			}
			super.tick();
		}
		...
		protected void eatBerry() {
			BlockState blockstate = ThescelosaurusEntity.this.world.getBlockState(this.destinationBlock);

			if (blockstate.isIn(PFBlocks.DEAD_OSMUNDACAULIS)) {
				int hunger = ThescelosaurusEntity.this.getCurrentHunger();
		 ...
		 */
		public boolean shouldExecute() {
			return !ThescelosaurusEntity.this.isAsleep() && super.shouldExecute() && ThescelosaurusEntity.this.getCurrentHunger() < ThescelosaurusEntity.this.getHalfHunger();
		}
