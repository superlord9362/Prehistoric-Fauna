...
public class ProtoceratopsSkeletonEntity extends PrehistoricEntity {

	private static final DataParameter<Boolean> DISPLAY = EntityDataManager.createKey(ProtoceratopsSkeletonEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> LAYING = EntityDataManager.createKey(ProtoceratopsSkeletonEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> SITTING = EntityDataManager.createKey(ProtoceratopsSkeletonEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> RUNNING = EntityDataManager.createKey(ProtoceratopsSkeletonEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> PUSHING = EntityDataManager.createKey(ProtoceratopsSkeletonEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> LOOKING = EntityDataManager.createKey(ProtoceratopsSkeletonEntity.class, DataSerializers.BOOLEAN);
	...
	private void setDisplaying(boolean isDisplaying) {
		this.dataManager.set(DISPLAY, isDisplaying);
	}

	public boolean isLaying() {
		return this.dataManager.get(LAYING);
	}

	private void setLaying(boolean isLaying) {
		this.dataManager.set(LAYING, isLaying);
	}
	...
	public ProtoceratopsSkeletonEntity(EntityType<? extends ProtoceratopsSkeletonEntity> type, World worldIn) {
		super(type, worldIn);
	}

	protected void registerData() {
		super.registerData();
		this.dataManager.register(DISPLAY, false);
		this.dataManager.register(LAYING, false);
	...
	public void writeAdditional(CompoundNBT compound) {
		...
		compound.putBoolean("isFlashing", this.isDisplaying());
		compound.putBoolean("isLayFighting", this.isLaying());
		compound.putBoolean("isStandFighting", this.isSitting());
	...
	public void readAdditional(CompoundNBT compound) {
		...
		this.setDisplaying(compound.getBoolean("isFlashing"));
		this.setLaying(compound.getBoolean("isLayFighting"));
		this.setSitting(compound.getBoolean("isStandFighting"));
	...
	public ActionResultType func_230254_b_(PlayerEntity player, Hand hand) {
		...
		if (itemstack.getItem() == PFItems.GEOLOGY_HAMMER.get()) {
			if (!this.isSleeping() && !this.isLaying() && !this.isSitting() && !this.isDisplaying() && !this.isRunning() && !player.isSneaking()) {
				this.setDisplaying(true);
			} else if (this.isDisplaying() && !player.isSneaking()) {
				this.setDisplaying(false);
				this.setRunning(true);
			} else if (this.isRunning() && !player.isSneaking()) {
				this.setRunning(false);
				this.setLaying(true);
			} else if (this.isLaying() && !player.isSneaking()) {
	...
	private void spawnFossil(DamageSource p_213815_1_) {
		Block.spawnAsEntity(this.world, new BlockPos(this.getPositionVec()), new ItemStack(PFItems.PROTOCERATOPS_SKELETON.get()));
	}
	
	static class LookAtPlayerGoal extends LookAtGoal {

		ProtoceratopsSkeletonEntity entity;
		
		public LookAtPlayerGoal(ProtoceratopsSkeletonEntity entityIn, Class<? extends LivingEntity> watchTargetClass, float maxDistance) {
