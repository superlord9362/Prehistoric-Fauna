...
public class TyrannosaurusSkullEntity extends PrehistoricEntity {
	...
	private static final DataParameter<Boolean> PUSHING = EntityDataManager.createKey(TyrannosaurusSkullEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> LOOKING = EntityDataManager.createKey(TyrannosaurusSkullEntity.class, DataSerializers.BOOLEAN);
	...
	private void spawnFossil(DamageSource p_213815_1_) {
		Block.spawnAsEntity(this.world, new BlockPos(this.getPositionVec()), new ItemStack(PFItems.TYRANNOSAURUS_SKULL.get()));
	}
	
	static class LookAtPlayerGoal extends LookAtGoal {

		TyrannosaurusSkullEntity entity;
		
		public LookAtPlayerGoal(TyrannosaurusSkullEntity entityIn, Class<? extends LivingEntity> watchTargetClass, float maxDistance) {
