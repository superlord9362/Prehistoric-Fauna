...
public class PlaceriasEntity extends DinosaurEntity {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(0, new PlaceriasEntity.NaturalMateGoal(this, 1.0D));
	...
	protected SoundEvent getAmbientSound() {
		return this.isAsleep() ? null : SoundInit.PLACERIAS_IDLE;
	}

	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return SoundInit.PLACERIAS_HURT;
	}

	protected SoundEvent getDeathSound() {
		return SoundInit.PLACERIAS_DEATH;
	}

	protected void playWarningSound() {
		if (this.warningSoundTicks <= 0) {
			this.playSound(SoundInit.PLACERIAS_WARN, 1.0F, this.getSoundPitch());
	...
	class AttackPlayerGoal extends NearestAttackableTargetGoal<PlayerEntity> {
		public AttackPlayerGoal() {
			super(PlaceriasEntity.this, PlayerEntity.class, 20, true, true, (Predicate<LivingEntity>)null);
		}

		public boolean shouldExecute() {
			if (PlaceriasEntity.this.isChild()) {
				return false;
			} else {
				if (super.shouldExecute()) {
					for(PlaceriasEntity placerias : PlaceriasEntity.this.world.getEntitiesWithinAABB(PlaceriasEntity.class, PlaceriasEntity.this.getBoundingBox().grow(8.0D, 4.0D, 8.0D))) {
	...
	class HurtByTargetGoal extends net.minecraft.entity.ai.goal.HurtByTargetGoal {
		...
		protected void setAttackTarget(MobEntity mobIn, LivingEntity targetIn) {
			if (mobIn instanceof PlaceriasEntity && !mobIn.isChild()) {
	...
	class MeleeAttackGoal extends net.minecraft.entity.ai.goal.MeleeAttackGoal {
		public MeleeAttackGoal() {
			super(PlaceriasEntity.this, 1.25D, true);
		}
		...
		protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
			...
			if (distToEnemySqr <= d0 && this.func_234040_h_()) {
				...
				if (this.func_234041_j_() <= 10) {
					PlaceriasEntity.this.playWarningSound();
				}
			} else {
				this.func_234039_g_();
			}
	...
	class PanicGoal extends net.minecraft.entity.ai.goal.PanicGoal {
		public PanicGoal() {
			super(PlaceriasEntity.this, 2.0D);
		}

		public boolean shouldExecute() {
			return !PlaceriasEntity.this.isChild() && !PlaceriasEntity.this.isBurning() ? false : super.shouldExecute();
		}
	}

	static class LayEggGoal extends MoveToBlockGoal {
		private final PlaceriasEntity placerias;

		LayEggGoal(PlaceriasEntity placerias, double speedIn) {
			super(placerias, speedIn, 16);
			this.placerias = placerias;
		}

		public boolean shouldExecute() {
			return this.placerias.hasEgg() ? super.shouldExecute() : false;
		}

		public boolean shouldContinueExecuting() {
			return super.shouldContinueExecuting() && this.placerias.hasEgg();
		}

		public void tick() {
			super.tick();
			BlockPos blockpos = new BlockPos(this.placerias.getPositionVec());
			if (!this.placerias.isInWater() && this.getIsAboveDestination()) {
	...
	static class MateGoal extends BreedGoal {
		...
		MateGoal(PlaceriasEntity placerias, double speedIn) {
			super(placerias, speedIn);
			this.placerias = placerias;
		}

		public boolean shouldExecute() {
			return super.shouldExecute() && !this.placerias.hasEgg() && !this.placerias.isInLoveNaturally();
		}
	...
	static class NaturalMateGoal extends BreedGoal {
		...
		NaturalMateGoal(PlaceriasEntity placerias, double speed) {
			super(placerias, speed);
			this.placerias = placerias;
		}

		public boolean shouldExecute() {
			return super.shouldExecute() && !this.placerias.hasEgg() && this.placerias.getCurrentHunger() >= this.placerias.getThreeQuartersHunger() && this.placerias.ticksExisted % 60 == 0 && (PrehistoricFaunaConfig.naturalEggBlockLaying || PrehistoricFaunaConfig.naturalEggItemLaying) && this.placerias.isInLoveNaturally();
		}

		protected void spawnBaby() {
			if (PrehistoricFaunaConfig.naturalEggItemLaying) {
				this.placerias.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.placerias.rand.nextFloat() - this.placerias.rand.nextFloat()) * 0.2F + 1.0F);
				int eggAmount = this.placerias.rand.nextInt(4);
				if (eggAmount == 0) {
					this.placerias.entityDropItem(PFBlocks.PLACERIAS_EGG.asItem());
				}
				if (eggAmount == 1) {
					this.placerias.entityDropItem(PFBlocks.PLACERIAS_EGG.asItem());
					this.placerias.entityDropItem(PFBlocks.PLACERIAS_EGG.asItem());
	...
	public AgeableEntity func_241840_a(ServerWorld p_241840_1_, AgeableEntity p_241840_2_) {
		PlaceriasEntity entity = new PlaceriasEntity(PFEntities.PLACERIAS_ENTITY, this.world);
	...
	public class HerbivoreEatGoal extends MoveToBlockGoal {
		...
		public HerbivoreEatGoal(double p_i50737_2_, int p_i50737_4_, int p_i50737_5_) {
			super(PlaceriasEntity.this, p_i50737_2_, p_i50737_4_, p_i50737_5_);
		}
		...
		public void tick() {
			if (this.getIsAboveDestination()) {
				if (this.field_220731_g >= 20) {
					...
					++this.field_220731_g;
					PlaceriasEntity.this.setEating(true);
				}
				if (this.field_220731_g % 5 == 1) {
					PlaceriasEntity.this.world.playSound((PlayerEntity)null, this.destinationBlock, SoundEvents.BLOCK_GRASS_HIT, SoundCategory.NEUTRAL, 1, 1);
				}
			}
			super.tick();
		}
		...
		protected void eatBerry() {
			BlockState blockstate = PlaceriasEntity.this.world.getBlockState(this.destinationBlock);
			int hunger = PlaceriasEntity.this.getCurrentHunger();

			if (blockstate.isIn(PFBlocks.DEAD_OSMUNDACAULIS)) {
				if (hunger + 2 >= PlaceriasEntity.this.maxHunger) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 2);
				}
			}
			if (blockstate.isIn(Blocks.GRASS) || blockstate.isIn(Blocks.TALL_GRASS) || blockstate.isIn(BlockTags.FLOWERS)) {
				if (hunger + 4 >= PlaceriasEntity.this.maxHunger) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 4);
				}
			}
			if (blockstate.isIn(Blocks.VINE)) {
				if (hunger + 6 >= PlaceriasEntity.this.maxHunger) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 6);
				}
			}
			if (blockstate.isIn(PFBlocks.MARCHANTIA) || blockstate.isIn(PFBlocks.OSMUNDACAULIS) || blockstate.isIn(PFBlocks.OTOZAMITES)) {
				if (hunger + 8 >= PlaceriasEntity.this.maxHunger) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 8);
				}
			}
			if (blockstate.isIn(PFBlocks.HORSETAIL) || blockstate.isIn(PFBlocks.CLUBMOSS) || blockstate.isIn(PFBlocks.MICHELILLOA) || blockstate.isIn(PFBlocks.COBBANIA) || blockstate.isIn(PFBlocks.LAUROZAMITES) || blockstate.isIn(PFBlocks.CLATHOPTERIS)) {
				if (hunger + 10 >= PlaceriasEntity.this.maxHunger) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 10);
				}
			}
			if (blockstate.isIn(PFBlocks.TALL_OSMUNDACAULIS) || blockstate.isIn(PFBlocks.TALL_OTOZAMITES)) {
				if (hunger + PlaceriasEntity.this.maxHunger >= 25) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 12);
				}
			}
			if (blockstate.isIn(PFBlocks.OSMUNDA) || blockstate.isIn(Blocks.FERN) || blockstate.isIn(PFBlocks.CONIOPTERIS) || blockstate.isIn(PFBlocks.CLADOPHLEBIS)) {
				if (hunger + 15 >= PlaceriasEntity.this.maxHunger) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 15);
				}
			}
			if (blockstate.isIn(PFBlocks.TALL_HORSETAIL) || blockstate.isIn(PFBlocks.SCYTOPHYLLUM)) {
				if (hunger + 20 >= PlaceriasEntity.this.maxHunger) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 20);
				}
			}
			if (blockstate.isIn(PFBlocks.TALL_OSMUNDA) || blockstate.isIn(Blocks.LARGE_FERN) || blockstate.isIn(PFBlocks.JOHNSTONIA)) {
				if (hunger + 25 >= PlaceriasEntity.this.maxHunger) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 25);
				}
			}
			if (blockstate.isIn(PFBlocks.DICROIDIUM)) {
				if (hunger + 30 >= PlaceriasEntity.this.maxHunger) {
					PlaceriasEntity.this.setHunger(PlaceriasEntity.this.maxHunger);
				} else {
					PlaceriasEntity.this.setHunger(hunger + 30);
				}
			}
			PlaceriasEntity.this.setEating(false);
		}
		 ...
		 */
		public boolean shouldExecute() {
			return !PlaceriasEntity.this.isAsleep() && super.shouldExecute() && PlaceriasEntity.this.getCurrentHunger() < PlaceriasEntity.this.getHalfHunger();
		}

		public boolean shouldContinueExecuting() {
			if (PlaceriasEntity.this.getCurrentHunger() >= PlaceriasEntity.this.maxHunger || PlaceriasEntity.this.isAsleep()) {
				return false;
