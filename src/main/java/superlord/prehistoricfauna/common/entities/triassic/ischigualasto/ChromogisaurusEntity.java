...
public class ChromogisaurusEntity extends DinosaurEntity {
	...
	private static final DataParameter<Boolean> EATING = EntityDataManager.createKey(ChromogisaurusEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> NATURAL_LOVE = EntityDataManager.createKey(ChromogisaurusEntity.class, DataSerializers.BOOLEAN);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(0, new ChromogisaurusEntity.MateGoal(this, 1.0D));
		this.goalSelector.addGoal(0, new ChromogisaurusEntity.NaturalMateGoal(this, 1.0D));
		...
		this.goalSelector.addGoal(7, new AvoidEntityGoal<DryosaurusEntity>(this, DryosaurusEntity.class, 10F, 1.75D, 1.5D));
		...
		this.goalSelector.addGoal(7, new AvoidEntityGoal<ThescelosaurusEntity>(this, ThescelosaurusEntity.class, 10F, 1.75D, 1.5D));
		this.goalSelector.addGoal(7, new AvoidEntityGoal<ExaeretodonEntity>(this, ExaeretodonEntity.class, 10F, 1.75D, 1.5D));
	...
	protected SoundEvent getAmbientSound() {
		return this.isAsleep() ? null : SoundInit.CHROMOGISAURUS_IDLE;
	}
	...
	static class LayEggGoal extends MoveToBlockGoal {
		...
		LayEggGoal(ChromogisaurusEntity chromogisaurus, double speedIn) {
		...
		public void tick() {
			...
			if (!this.chromogisaurus.isInWater() && this.getIsAboveDestination()) {
				if (this.chromogisaurus.isDigging < 1) {
					...
					world.setBlockState(this.destinationBlock.up(), PFBlocks.CHROMOGISAURUS_EGG.getDefaultState().with(DinosaurEggBlock.EGGS, Integer.valueOf(this.chromogisaurus.rand.nextInt(4) + 1)), 3);
	...
	static class MateGoal extends BreedGoal {
		...
		public boolean shouldExecute() {
			return super.shouldExecute() && !this.chromogisaurus.hasEgg() && !this.chromogisaurus.isInLoveNaturally();
		}
	...
	static class NaturalMateGoal extends BreedGoal {
		...
		NaturalMateGoal(ChromogisaurusEntity chromogisaurus, double speed) {
			super(chromogisaurus, speed);
			this.chromogisaurus = chromogisaurus;
		}

		public boolean shouldExecute() {
			return super.shouldExecute() && !this.chromogisaurus.hasEgg() && this.chromogisaurus.getCurrentHunger() >= this.chromogisaurus.getThreeQuartersHunger() && this.chromogisaurus.ticksExisted % 60 == 0 && (PrehistoricFaunaConfig.naturalEggBlockLaying || PrehistoricFaunaConfig.naturalEggItemLaying) && this.chromogisaurus.isInLoveNaturally();
		}

		protected void spawnBaby() {
			if (PrehistoricFaunaConfig.naturalEggItemLaying) {
				this.chromogisaurus.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.chromogisaurus.rand.nextFloat() - this.chromogisaurus.rand.nextFloat()) * 0.2F + 1.0F);
				int eggAmount = this.chromogisaurus.rand.nextInt(4);
				if (eggAmount == 0) {
					this.chromogisaurus.entityDropItem(PFBlocks.CHROMOGISAURUS_EGG.asItem());
				}
				if (eggAmount == 1) {
					this.chromogisaurus.entityDropItem(PFBlocks.CHROMOGISAURUS_EGG.asItem());
					this.chromogisaurus.entityDropItem(PFBlocks.CHROMOGISAURUS_EGG.asItem());
	...
	public class HerbivoreEatGoal extends MoveToBlockGoal {
		...
		public HerbivoreEatGoal(double p_i50737_2_, int p_i50737_4_, int p_i50737_5_) {
			super(ChromogisaurusEntity.this, p_i50737_2_, p_i50737_4_, p_i50737_5_);
		}
		...
		public void tick() {
			if (this.getIsAboveDestination()) {
				if (this.field_220731_g >= 20) {
					...
					++this.field_220731_g;
					ChromogisaurusEntity.this.setEating(true);
				}
				if (this.field_220731_g % 5 == 1) {
					ChromogisaurusEntity.this.world.playSound((PlayerEntity)null, this.destinationBlock, SoundEvents.BLOCK_GRASS_HIT, SoundCategory.NEUTRAL, 1, 1);
				}
			}
			super.tick();
		}
		...
		protected void eatBerry() {
			BlockState blockstate = ChromogisaurusEntity.this.world.getBlockState(this.destinationBlock);
			int hunger = ChromogisaurusEntity.this.getCurrentHunger();

			if (blockstate.isIn(PFBlocks.DEAD_OSMUNDACAULIS)) {
				if (hunger + 2 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 2);
				}
			}
			if (blockstate.isIn(Blocks.GRASS) || blockstate.isIn(Blocks.TALL_GRASS) || blockstate.isIn(BlockTags.FLOWERS)) {
				if (hunger + 4 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 4);
				}
			}
			if (blockstate.isIn(Blocks.VINE)) {
				if (hunger + 6 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 6);
				}
			}
			if (blockstate.isIn(PFBlocks.MARCHANTIA) || blockstate.isIn(PFBlocks.OSMUNDACAULIS) || blockstate.isIn(PFBlocks.OTOZAMITES)) {
				if (hunger + 8 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 8);
				}
			}
			if (blockstate.isIn(PFBlocks.HORSETAIL) || blockstate.isIn(PFBlocks.CLUBMOSS) || blockstate.isIn(PFBlocks.MICHELILLOA) || blockstate.isIn(PFBlocks.COBBANIA) || blockstate.isIn(PFBlocks.LAUROZAMITES) || blockstate.isIn(PFBlocks.CLATHOPTERIS)) {
				if (hunger + 10 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 10);
				}
			}
			if (blockstate.isIn(PFBlocks.TALL_OSMUNDACAULIS) || blockstate.isIn(PFBlocks.TALL_OTOZAMITES)) {
				if (hunger + 12 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 12);
				}
			}
			if (blockstate.isIn(PFBlocks.OSMUNDA) || blockstate.isIn(Blocks.FERN) || blockstate.isIn(PFBlocks.CONIOPTERIS) || blockstate.isIn(PFBlocks.CLADOPHLEBIS)) {
				if (hunger + 15 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 15);
				}
			}
			if (blockstate.isIn(PFBlocks.TALL_HORSETAIL) || blockstate.isIn(PFBlocks.SCYTOPHYLLUM)) {
				if (hunger + 20 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 20);
				}
			}
			if (blockstate.isIn(PFBlocks.TALL_OSMUNDA) || blockstate.isIn(Blocks.LARGE_FERN) || blockstate.isIn(PFBlocks.JOHNSTONIA)) {
				if (hunger + 25 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 25);
				}
			}
			if (blockstate.isIn(PFBlocks.DICROIDIUM)) {
				if (hunger + 30 >= ChromogisaurusEntity.this.maxHunger) {
					ChromogisaurusEntity.this.setHunger(ChromogisaurusEntity.this.maxHunger);
				} else {
					ChromogisaurusEntity.this.setHunger(hunger + 30);
				}
			}
			ChromogisaurusEntity.this.setEating(false);
		}
		 ...
		 */
		public boolean shouldExecute() {
			return !ChromogisaurusEntity.this.isAsleep() && super.shouldExecute() && ChromogisaurusEntity.this.getCurrentHunger() < ChromogisaurusEntity.this.getHalfHunger();
		}

		public boolean shouldContinueExecuting() {
			if (ChromogisaurusEntity.this.getCurrentHunger() >= ChromogisaurusEntity.this.maxHunger || ChromogisaurusEntity.this.isAsleep()) {
				return false;
	...
	public class CarnivoreHuntGoal extends NearestAttackableTargetGoal {
		...
		public boolean shouldExecute() {
			return super.shouldExecute() && ChromogisaurusEntity.this.getCurrentHunger() <= ChromogisaurusEntity.this.getHalfHunger() && PrehistoricFaunaConfig.advancedHunger == true;
		}

		public boolean shouldContinueExecuting() {
			return ChromogisaurusEntity.this.getCurrentHunger() < ChromogisaurusEntity.this.maxHunger && PrehistoricFaunaConfig.advancedHunger == true;
		}
