...
public class PoposaurusEntity extends DinosaurEntity {
	...
	class MeleeAttackGoal extends net.minecraft.entity.ai.goal.MeleeAttackGoal {
		...
		protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
			...
			if (distToEnemySqr <= d0 && this.func_234040_h_()) {
				this.func_234039_g_();
				PoposaurusEntity.this.playSound(SoundInit.POPOSAURUS_BITE, 1.0F, PoposaurusEntity.this.getSoundPitch());
		...
		public void tick() {
			...
			if (!this.poposaurus.isInWater() && this.getIsAboveDestination()) {
				if (this.poposaurus.isDigging < 1) {
					...
					world.setBlockState(this.destinationBlock.up(), PFBlocks.POPOSAURUS_EGG.getDefaultState().with(DinosaurEggBlock.EGGS, Integer.valueOf(this.poposaurus.rand.nextInt(4) + 1)), 3);
	...
	public class CarnivoreHuntGoal extends NearestAttackableTargetGoal {
		...
		public boolean shouldExecute() {
			return super.shouldExecute() && PoposaurusEntity.this.getCurrentHunger() <= PoposaurusEntity.this.getHalfHunger() && !PoposaurusEntity.this.isChild() && PrehistoricFaunaConfig.advancedHunger == true;
		}

		public boolean shouldContinueExecuting() {
			return PoposaurusEntity.this.getCurrentHunger() < PoposaurusEntity.this.maxHunger && PrehistoricFaunaConfig.advancedHunger == true;
		}
		...
		public void tick() {
			...
			if (target instanceof VelociraptorEntity || target instanceof BasilemysEntity || target instanceof ExaeretodonEntity || target instanceof SheepEntity || target instanceof WolfEntity) {
				if (target.getHealth() == 0) {
			...
			if (target instanceof ThescelosaurusEntity || target instanceof SarahsaurusEntity || target instanceof ScelidosaurusEntity || target instanceof DryosaurusEntity || target instanceof CitipatiEntity) {
				if (target.getHealth() == 0) {
			...
			if (target instanceof DesmatosuchusEntity || target instanceof PlaceriasEntity || target instanceof HerrerasaurusEntity) {
				if (target.getHealth() == 0) {
					if (PoposaurusEntity.this.getCurrentHunger() + 20 >= PoposaurusEntity.this.maxHunger) {
						PoposaurusEntity.this.setHunger(PoposaurusEntity.this.maxHunger);
					} else {
						PoposaurusEntity.this.setHunger(currentHunger + 20);
					}
				}
			}
			super.tick();
	...
	public class BabyCarnivoreHuntGoal extends NearestAttackableTargetGoal {
		...
		public boolean shouldContinueExecuting() {
			return PoposaurusEntity.this.getCurrentHunger() < PoposaurusEntity.this.maxHunger && PrehistoricFaunaConfig.advancedHunger == true || !PoposaurusEntity.this.isChild() && PrehistoricFaunaConfig.advancedHunger == true;
		}
