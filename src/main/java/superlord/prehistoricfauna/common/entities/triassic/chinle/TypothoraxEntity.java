...
public class TypothoraxEntity extends DinosaurEntity {
	...
	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return SoundInit.TYPOTHORAX_HURT;
	}
	...
	static class LayEggGoal extends MoveToBlockGoal {
		...
		LayEggGoal(TypothoraxEntity typothorax, double speedIn) {
		...
		public boolean shouldExecute() {
			return this.typothorax.hasEgg() ? super.shouldExecute() : false;
		}
		...
		public boolean shouldContinueExecuting() {
			return super.shouldContinueExecuting() && this.typothorax.hasEgg();
		}
		...
		public void tick() {
			super.tick();
			BlockPos blockpos = new BlockPos(this.typothorax.getPositionVec());
			if (!this.typothorax.isInWater() && this.getIsAboveDestination()) {
	...
	static class MateGoal extends BreedGoal {
		...
		MateGoal(TypothoraxEntity typothorax, double speedIn) {
		 ...
		 */
		public boolean shouldExecute() {
			return super.shouldExecute() && !this.typothorax.hasEgg() && !this.typothorax.isInLoveNaturally();
		}
	...
	static class NaturalMateGoal extends BreedGoal {
		...
		NaturalMateGoal(TypothoraxEntity typothorax, double speed) {
			super(typothorax, speed);
			this.typothorax = typothorax;
		}

		public boolean shouldExecute() {
			return super.shouldExecute() && !this.typothorax.hasEgg() && this.typothorax.getCurrentHunger() >= this.typothorax.getThreeQuartersHunger() && this.typothorax.ticksExisted % 60 == 0 && (PrehistoricFaunaConfig.naturalEggBlockLaying || PrehistoricFaunaConfig.naturalEggItemLaying) && this.typothorax.isInLoveNaturally();
		}

		protected void spawnBaby() {
			if (PrehistoricFaunaConfig.naturalEggItemLaying) {
				this.typothorax.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.typothorax.rand.nextFloat() - this.typothorax.rand.nextFloat()) * 0.2F + 1.0F);
				int eggAmount = this.typothorax.rand.nextInt(4);
				if (eggAmount == 0) {
					this.typothorax.entityDropItem(PFBlocks.TYPOTHORAX_EGG.asItem());
				}
				if (eggAmount == 1) {
					this.typothorax.entityDropItem(PFBlocks.TYPOTHORAX_EGG.asItem());
					this.typothorax.entityDropItem(PFBlocks.TYPOTHORAX_EGG.asItem());
	...
	public AgeableEntity func_241840_a(ServerWorld p_241840_1_, AgeableEntity p_241840_2_) {
		TypothoraxEntity entity = new TypothoraxEntity(PFEntities.TYPOTHORAX_ENTITY, this.world);
	...
	public class HerbivoreEatGoal extends MoveToBlockGoal {
		...
		public HerbivoreEatGoal(double p_i50737_2_, int p_i50737_4_, int p_i50737_5_) {
			super(TypothoraxEntity.this, p_i50737_2_, p_i50737_4_, p_i50737_5_);
		}
		...
		public void tick() {
			if (this.getIsAboveDestination()) {
				if (this.field_220731_g >= 20) {
					...
					++this.field_220731_g;
					TypothoraxEntity.this.setEating(true);
				}
				if (this.field_220731_g % 5 == 1) {
					TypothoraxEntity.this.world.playSound((PlayerEntity)null, this.destinationBlock, SoundEvents.BLOCK_GRASS_HIT, SoundCategory.NEUTRAL, 1, 1);
				}
			}
			super.tick();
		}
		...
		protected void eatBerry() {
			BlockState blockstate = TypothoraxEntity.this.world.getBlockState(this.destinationBlock);

			if (blockstate.isIn(PFBlocks.DEAD_OSMUNDACAULIS)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 2 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 2);
					TypothoraxEntity.this.setEating(false);
				}
			}
			if (blockstate.isIn(Blocks.GRASS) || blockstate.isIn(Blocks.TALL_GRASS) || blockstate.isIn(BlockTags.FLOWERS)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 4 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 4);
					TypothoraxEntity.this.setEating(false);
				}
			}
			if (blockstate.isIn(Blocks.VINE)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 6 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 6);
					TypothoraxEntity.this.setEating(false);
				}
			}
			if (blockstate.isIn(PFBlocks.MARCHANTIA) || blockstate.isIn(PFBlocks.OSMUNDACAULIS) || blockstate.isIn(PFBlocks.OTOZAMITES)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 8 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 8);
					TypothoraxEntity.this.setEating(false);
				}
			}
			if (blockstate.isIn(PFBlocks.HORSETAIL) || blockstate.isIn(PFBlocks.CLUBMOSS) || blockstate.isIn(PFBlocks.MICHELILLOA) || blockstate.isIn(PFBlocks.COBBANIA) || blockstate.isIn(PFBlocks.LAUROZAMITES) || blockstate.isIn(PFBlocks.CLATHOPTERIS)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 10 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 10);
					TypothoraxEntity.this.setEating(false);
				}
			}
			if (blockstate.isIn(PFBlocks.TALL_OSMUNDACAULIS) || blockstate.isIn(PFBlocks.TALL_OTOZAMITES)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 12 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 12);
					TypothoraxEntity.this.setEating(false);
				}
			}
			if (blockstate.isIn(PFBlocks.OSMUNDA) || blockstate.isIn(Blocks.FERN) || blockstate.isIn(PFBlocks.CONIOPTERIS) || blockstate.isIn(PFBlocks.CLADOPHLEBIS)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 15 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 15);
					TypothoraxEntity.this.setEating(false);
				}
			}
			if (blockstate.isIn(PFBlocks.TALL_HORSETAIL) || blockstate.isIn(PFBlocks.SCYTOPHYLLUM)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 20 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 20);
					TypothoraxEntity.this.setEating(false);
				}
			}
			if (blockstate.isIn(PFBlocks.TALL_OSMUNDA) || blockstate.isIn(Blocks.LARGE_FERN) || blockstate.isIn(PFBlocks.JOHNSTONIA)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 25 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 25);
					TypothoraxEntity.this.setEating(false);
				}
			}
			if (blockstate.isIn(PFBlocks.DICROIDIUM)) {
				int hunger = TypothoraxEntity.this.getCurrentHunger();
				if (hunger + 30 >= TypothoraxEntity.this.maxHunger) {
					TypothoraxEntity.this.setHunger(TypothoraxEntity.this.maxHunger);
					TypothoraxEntity.this.setEating(false);
				} else {
					TypothoraxEntity.this.setHunger(hunger + 30);
					TypothoraxEntity.this.setEating(false);
				}
			}
		}
		 ...
		 */
		public boolean shouldExecute() {
			return !TypothoraxEntity.this.isAsleep() && super.shouldExecute() && TypothoraxEntity.this.getCurrentHunger() < TypothoraxEntity.this.getHalfHunger();
		}

		public boolean shouldContinueExecuting() {
			if (TypothoraxEntity.this.getCurrentHunger() >= TypothoraxEntity.this.maxHunger || TypothoraxEntity.this.isAsleep()) {
				return false;
