...
public class WallClimbingBug extends Animal {
	private static final EntityDataAccessor<Direction> ATTACHED_FACE = SynchedEntityData.defineId(WallClimbingBug.class, EntityDataSerializers.DIRECTION);
	private static final EntityDataAccessor<Byte> CLIMBING = SynchedEntityData.defineId(WallClimbingBug.class, EntityDataSerializers.BYTE);
	...
	public WallClimbingBug(EntityType type, Level world) {
		super(type, world);
	    this.setPathfindingMalus(BlockPathTypes.WATER, -1.0F);
	    this.setPathfindingMalus(BlockPathTypes.UNPASSABLE_RAIL, 0.0F);
	    this.setPathfindingMalus(BlockPathTypes.FENCE, 0.0F);
	    this.setPathfindingMalus(BlockPathTypes.LEAVES, 0.0F);
	    this.setPathfindingMalus(BlockPathTypes.COCOA, 0.0F);
	...
	private void switchNavigator(boolean rightsideUp) {
		if (rightsideUp) {
			...
			this.navigation = new BugNavigation(this, level());
	...
	protected PathNavigation createNavigation(Level worldIn) {
		return new BugNavigation(this, worldIn);
	}
	...
	public class BugNavigation extends WallClimberNavigation {
	    
	    public BugNavigation(Mob mob, Level level) {
	        super(mob, level);
	    }
	    ...
	    public boolean isStableDestination(BlockPos pos) {
	        ...
	        if (state.isSuffocating(this.level, pos)) return false;
	        return !this.level.isEmptyBlock(pos.below())
	            || !this.level.isEmptyBlock(pos.north())
	            || !this.level.isEmptyBlock(pos.south())
	            || !this.level.isEmptyBlock(pos.east())
	            || !this.level.isEmptyBlock(pos.west())
	    ...
	    public boolean moveTo(double x, double y, double z, double speed) {
	        ...
	        if (this.level.isEmptyBlock(target.below())) {
	            for (int i = 1; i <= 3; i++) {
	                if (!this.level.isEmptyBlock(target.below(i))) {
	                    y = target.below(i - 1).getY();
	                    break;
	                }
	            }
	        }
