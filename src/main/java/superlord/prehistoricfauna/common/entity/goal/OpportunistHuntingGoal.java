...
public class OpportunistHuntingGoal extends NearestAttackableTargetGoal {
	double huntSpeed;
	Predicate<LivingEntity> targetPredicate;
	DinosaurEntity dinosaur;

	@SuppressWarnings("unchecked")
	public OpportunistHuntingGoal(DinosaurEntity goalOwnerIn, Class targetClassIn, int targetChanceIn, double huntSpeed, boolean checkSight, boolean nearbyOnly, @Nullable Predicate<LivingEntity> targetPredicate) {
	...
	public boolean canUse() {
		return super.canUse() && dinosaur.getCurrentHunger() <= dinosaur.getHalfHunger() && !dinosaur.isBaby() && PrehistoricFaunaConfig.advancedHunger == true && !targetPredicate.test(dinosaur) && !dinosaur.trusts(target.getUUID()) && (target.getHealth() <= target.getMaxHealth() && target.getHealth() < dinosaur.getHealth() || target.getMaxHealth() < dinosaur.getHealth()) && dinosaur.isOpportunist();
	}
