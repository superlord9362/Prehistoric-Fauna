...
public class Basilemys extends DinosaurEntity {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(8, new AvoidEntityGoal<LivingEntity>(this, LivingEntity.class, 7F, 1.5D, 1.75D, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.BASILEMYS_AVOIDING);
		}));
	}
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if (item instanceof PaleopediaItem) {
            ...
            if (!already.contains(EnumPaleoPages.BASILEMYS.ordinal())) {
				EnumPaleoPages.addPage(EnumPaleoPages.fromInt(EnumPaleoPages.BASILEMYS.ordinal()), itemstack);
				player.displayClientMessage(Component.translatable("paleopedia.basilemys_added"), true);
				return InteractionResult.SUCCESS;
			} else {
				player.displayClientMessage(Component.translatable("paleopedia.basilemys_already_added"), true);
				return InteractionResult.SUCCESS;
			}
		}
	...
	public BlockState getEggBlock(Level world, BlockPos pos) {
		return PFBlocks.BASILEMYS_EGG.get().defaultBlockState().setValue(DinosaurEggBlock.EGGS, Integer.valueOf(this.random.nextInt(4) + 1));
	}
}