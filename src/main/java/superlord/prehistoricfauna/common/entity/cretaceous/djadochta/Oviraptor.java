...
public class Oviraptor extends DinosaurEntity {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(8, new AvoidEntityGoal<LivingEntity>(this, LivingEntity.class, 7F, 1.5D, 1.75D, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.OVIRAPTOR_AVOIDING);
		}));
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if (item instanceof PaleopediaItem) {
            ...
            if (!already.contains(EnumPaleoPages.OVIRAPTOR.ordinal())) {
				EnumPaleoPages.addPage(EnumPaleoPages.fromInt(EnumPaleoPages.OVIRAPTOR.ordinal()), itemstack);
				player.displayClientMessage(Component.translatable("paleopedia.oviraptor_added"), true);
				return InteractionResult.SUCCESS;
			} else {
				player.displayClientMessage(Component.translatable("paleopedia.oviraptor_already_added"), true);
				return InteractionResult.SUCCESS;
			}
		}
