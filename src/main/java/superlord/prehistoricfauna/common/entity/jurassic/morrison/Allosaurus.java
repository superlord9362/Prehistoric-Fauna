...
public class Allosaurus extends DinosaurEntity {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(8, new AvoidEntityGoal<LivingEntity>(this, LivingEntity.class, 7F, 1.5D, 1.75D, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.ALLOSAURUS_AVOIDING);
		}));
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if (item instanceof PaleopediaItem) {
            ...
            if (!already.contains(EnumPaleoPages.ALLOSAURUS.ordinal())) {
				EnumPaleoPages.addPage(EnumPaleoPages.fromInt(EnumPaleoPages.ALLOSAURUS.ordinal()), itemstack);
				player.displayClientMessage(Component.translatable("paleopedia.allosaurus_added"), true);
				return InteractionResult.SUCCESS;
			} else {
				player.displayClientMessage(Component.translatable("paleopedia.allosaurus_already_added"), true);
				return InteractionResult.SUCCESS;
			}
		}
	...
	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return PFSounds.ALLOSAURUS_HURT.get();
	}

	protected SoundEvent getDeathSound() {
		return PFSounds.ALLOSAURUS_DEATH.get();
	}
	...
	protected void playWarningSound() {
		if (this.warningSoundTicks <= 0) {
			this.playSound(PFSounds.ALLOSAURUS_WARN.get(), 1.0F, this.getVoicePitch());
	...
	class MeleeAttackGoal extends net.minecraft.world.entity.ai.goal.MeleeAttackGoal {
		...
		protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
			...
			if (distToEnemySqr <= d0 && this.isTimeToAttack()) {
				...
				Allosaurus.this.playSound(PFSounds.ALLOSAURUS_BITE.get(), 1.0F, Allosaurus.this.getVoicePitch());
	...
	public BlockState getEggBlock(Level world, BlockPos pos) {
		return PFBlocks.ALLOSAURUS_EGG.get().defaultBlockState().setValue(DinosaurEggBlock.EGGS, Integer.valueOf(this.random.nextInt(4) + 1));
	}
