...
public class Huayangosaurus extends DinosaurEntity {
	private int maxHunger = 38;
	private int warningSoundTicks;

	public Huayangosaurus(EntityType<? extends Huayangosaurus> type, Level levelIn) {
		super(type, levelIn);
	...
	public boolean isFood(ItemStack stack) {
		return stack.getItem() == PFBlocks.TAENIOPTERIS.get().asItem();
	}

	protected void registerGoals() {
		super.registerGoals();
		this.goalSelector.addGoal(0, new FloatGoal(this));
		this.goalSelector.addGoal(1, new Huayangosaurus.MeleeAttackGoal());
		...
		this.goalSelector.addGoal(8, new AvoidEntityGoal<LivingEntity>(this, LivingEntity.class, 7F, 1.5D, 1.75D, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.HUAYANGOSAURUS_AVOIDING);
		}));
	}
	...
	public static AttributeSupplier.Builder createAttributes() {
		return Mob.createMobAttributes().add(Attributes.MAX_HEALTH, 15.0D).add(Attributes.FOLLOW_RANGE, 20.0D).add(Attributes.MOVEMENT_SPEED, 0.2D).add(Attributes.ATTACK_DAMAGE, 2).add(Attributes.KNOCKBACK_RESISTANCE, 0.5D);
	}

	protected SoundEvent getAmbientSound() {
		return this.isAsleep() ? null : PFSounds.HUAYANGOSAURUS_IDLE.get();
	}

	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return PFSounds.HUAYANGOSAURUS_HURT.get();
	}

	protected SoundEvent getDeathSound() {
		return PFSounds.HUAYANGOSAURUS_DEATH.get();
	}
	...
	protected void playWarningSound() {
		if (this.warningSoundTicks <= 0) {
			this.playSound(PFSounds.HUAYANGOSAURUS_WARN.get(), 1.0F, this.getVoicePitch());
	...
	class MeleeAttackGoal extends net.minecraft.world.entity.ai.goal.MeleeAttackGoal {
		public MeleeAttackGoal() {
			super(Huayangosaurus.this, 1.25D, true);
		}
		...
		protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
			...
			if (distToEnemySqr <= d0 && this.isTimeToAttack()) {
				...
				if (this.getTicksUntilNextAttack() <= 10) {
					Huayangosaurus.this.playWarningSound();
				}
			} else {
				this.resetAttackCooldown();
			}
	...
	class PanicGoal extends net.minecraft.world.entity.ai.goal.PanicGoal {
		public PanicGoal() {
			super(Huayangosaurus.this, 2.0D);
		}
		...
		public boolean canUse() {
			return !Huayangosaurus.this.isBaby() && !Huayangosaurus.this.isOnFire() ? false : super.canUse();
		}
	}

	@Override
	public AgeableMob getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
		Huayangosaurus entity = new Huayangosaurus(PFEntities.HUAYANGOSAURUS.get(), this.level());
	...
	public ItemStack getPickedResult(HitResult target) {
		return new ItemStack(PFItems.HUAYANGOSAURUS_SPAWN_EGG.get());
	}
	
	public Item getEggItem() {
		return PFItems.HUAYANGOSAURUS_EGG.get();
	}
    
	public BlockState getEggBlock(Level world, BlockPos pos) {
		return PFBlocks.HUAYANGOSAURUS_EGG.get().defaultBlockState().setValue(DinosaurEggBlock.EGGS, Integer.valueOf(this.random.nextInt(4) + 1));
	}
}
