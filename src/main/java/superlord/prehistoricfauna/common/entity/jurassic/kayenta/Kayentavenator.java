...
public class Kayentavenator extends DinosaurEntity {
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(4, new HuntGoal(this, LivingEntity.class, 10, false, false, (p_213487_1_) -> {
			return p_213487_1_.getType().is(PFTags.KAYENTAVENATOR_HUNTING);
		}));
		this.goalSelector.addGoal(1, new Kayentavenator.MeleeAttackGoal());
		...
		this.goalSelector.addGoal(8, new AvoidEntityGoal<LivingEntity>(this, LivingEntity.class, 7F, 1.5D, 1.75D, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.KAYENTAVENATOR_AVOIDING);
		}));
	...
	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return PFSounds.KAYENTAVENATOR_HURT.get();
	}

	protected SoundEvent getDeathSound() {
		return PFSounds.KAYENTAVENATOR_DEATH.get();
	}

	protected void playWarningSound() {
		if (this.warningSoundTicks <= 0) {
			this.playSound(PFSounds.KAYENTAVENATOR_WARN.get(), 1.0F, this.getVoicePitch());
	...
	class MeleeAttackGoal extends net.minecraft.world.entity.ai.goal.MeleeAttackGoal {
		public MeleeAttackGoal() {
			super(Kayentavenator.this, 1.25D, true);
		}
		...
		protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
			...
			if (distToEnemySqr <= d0 && this.isTimeToAttack()) {
				...
				if (this.getTicksUntilNextAttack() <= 10) {
					Kayentavenator.this.playWarningSound();
				}
			} else {
				this.resetAttackCooldown();
			}
	...
	public AgeableMob getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
		Kayentavenator entity = new Kayentavenator(PFEntities.KAYENTAVENATOR.get(), this.level());
