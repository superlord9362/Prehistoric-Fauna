...
public class Bashanosaurus extends DinosaurEntity {
	private int maxHunger = 38;
	private int warningSoundTicks;

	public Bashanosaurus(EntityType<? extends Bashanosaurus> type, Level levelIn) {
		super(type, levelIn);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new Bashanosaurus.MeleeAttackGoal());
		...
		this.goalSelector.addGoal(8, new AvoidEntityGoal<LivingEntity>(this, LivingEntity.class, 7F, 1.5D, 1.75D, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.BASHANOSAURUS_AVOIDING);
		}));
	}
	...
	protected SoundEvent getAmbientSound() {
		return this.isAsleep() ? null : PFSounds.BASHANOSAURUS_IDLE.get();
	}

	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return PFSounds.BASHANOSAURUS_HURT.get();
	}

	protected SoundEvent getDeathSound() {
		return PFSounds.BASHANOSAURUS_DEATH.get();
	}
	...
	protected void playWarningSound() {
		if (this.warningSoundTicks <= 0) {
			this.playSound(PFSounds.BASHANOSAURUS_WARN.get(), 1.0F, this.getVoicePitch());
	...
	class MeleeAttackGoal extends net.minecraft.world.entity.ai.goal.MeleeAttackGoal {
		public MeleeAttackGoal() {
			super(Bashanosaurus.this, 1.25D, true);
		}
		...
		protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
			...
			if (distToEnemySqr <= d0 && this.isTimeToAttack()) {
				...
				if (this.getTicksUntilNextAttack() <= 10) {
					Bashanosaurus.this.playWarningSound();
				}
			} else {
				this.resetAttackCooldown();
			}
	...
	class PanicGoal extends net.minecraft.world.entity.ai.goal.PanicGoal {
		...
		public boolean canUse() {
			return !Bashanosaurus.this.isBaby() && !Bashanosaurus.this.isOnFire() ? false : super.canUse();
		}
	}

	@Override
	public AgeableMob getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
		Bashanosaurus entity = new Bashanosaurus(PFEntities.BASHANOSAURUS.get(), this.level());
	...
	public ItemStack getPickedResult(HitResult target) {
		return new ItemStack(PFItems.BASHANOSAURUS_SPAWN_EGG.get());
	}
	
	public Item getEggItem() {
		return PFItems.BASHANOSAURUS_EGG.get();
	}
    
	public BlockState getEggBlock(Level world, BlockPos pos) {
		return PFBlocks.BASHANOSAURUS_EGG.get().defaultBlockState().setValue(DinosaurEggBlock.EGGS, Integer.valueOf(this.random.nextInt(4) + 1));
	}
}
