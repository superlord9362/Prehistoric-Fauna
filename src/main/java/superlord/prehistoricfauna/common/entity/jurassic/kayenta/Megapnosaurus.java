...
public class Megapnosaurus extends DinosaurEntity {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(8, new AvoidEntityGoal<LivingEntity>(this, LivingEntity.class, 7F, 1.5D, 1.75D, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.MEGAPNOSAURUS_AVOIDING);
		}));
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if (item instanceof PaleopediaItem paleopedia) {
			if (!itemstack.getTag().contains("Pages", EnumPaleoPages.MEGAPNOSAURUS.ordinal())) {
				EnumPaleoPages.addPage(EnumPaleoPages.fromInt(EnumPaleoPages.MEGAPNOSAURUS.ordinal()), itemstack);
				player.displayClientMessage(Component.translatable("paleopedia.megapnosaurus_added"), true);
	...
	protected SoundEvent getAmbientSound() {
		return this.isAsleep() ? null : PFSounds.MEGAPNOSAURUS_IDLE.get();
	}

	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return PFSounds.MEGAPNOSAURUS_HURT.get();
	}

	protected SoundEvent getDeathSound() {
		return PFSounds.MEGAPNOSAURUS_DEATH.get();
	}

	protected void playWarningSound() {
		if (this.warningSoundTicks <= 0) {
			this.playSound(PFSounds.MEGAPNOSAURUS_WARN.get(), 1.0F, this.getVoicePitch());
	...
	public BlockState getEggBlock(Level world, BlockPos pos) {
		return PFBlocks.MEGAPNOSAURUS_NEST.get().defaultBlockState().setValue(NestAndEggsBlock.EGGS, Integer.valueOf(this.random.nextInt(4) + 1)).setValue(NestAndEggsBlock.PLANT_LEVEL, Integer.valueOf(this.random.nextInt(3) + 1));
	}
