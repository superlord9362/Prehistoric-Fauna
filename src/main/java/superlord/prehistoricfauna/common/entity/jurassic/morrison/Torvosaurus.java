...
public class Torvosaurus extends DinosaurEntity {
	private int maxHunger = 200;
	private int warningSoundTicks;

	public Torvosaurus(EntityType<? extends Torvosaurus> type, Level worldIn) {
		super(type, worldIn);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(4, new HuntGoal(this, LivingEntity.class, 10, false, false, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.TORVOSAURUS_HUNTING);
		}));
		this.goalSelector.addGoal(1, new Torvosaurus.MeleeAttackGoal());
		...
		this.goalSelector.addGoal(8, new AvoidEntityGoal<LivingEntity>(this, LivingEntity.class, 7F, 1.5D, 1.75D, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.TORVOSAURUS_AVOIDING);
		}));
		...
		this.targetSelector.addGoal(0, new BabyCarnivoreHuntGoal(this, LivingEntity.class, 10, 1.75D, true, false, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.TORVOSAURUS_BABY_HUNTING);
		}));
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if (item instanceof PaleopediaItem) {
            ...
            if (!already.contains(EnumPaleoPages.TORVOSAURUS.ordinal())) {
				EnumPaleoPages.addPage(EnumPaleoPages.fromInt(EnumPaleoPages.TORVOSAURUS.ordinal()), itemstack);
				player.displayClientMessage(Component.translatable("paleopedia.torvosaurus_added"), true);
				return InteractionResult.SUCCESS;
			} else {
				player.displayClientMessage(Component.translatable("paleopedia.torvosaurus_already_added"), true);
				return InteractionResult.SUCCESS;
			}
		}
	...
	protected SoundEvent getAmbientSound() {
		return this.isAsleep() ? PFSounds.TORVOSAURUS_SNORES.get() : PFSounds.TORVOSAURUS_IDLE.get();
	}

	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return PFSounds.TORVOSAURUS_HURT.get();
	}

	protected SoundEvent getDeathSound() {
		return PFSounds.TORVOSAURUS_DEATH.get();
	}
	...
	protected void playWarningSound() {
		if (this.warningSoundTicks <= 0) {
			this.playSound(PFSounds.TORVOSAURUS_WARN.get(), 1.0F, this.getVoicePitch());
	...
	class MeleeAttackGoal extends net.minecraft.world.entity.ai.goal.MeleeAttackGoal {
		public MeleeAttackGoal() {
			super(Torvosaurus.this, 1.25D, true);
		}
		...
		protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
			...
			if (distToEnemySqr <= d0 && this.isTimeToAttack()) {
				...
				Torvosaurus.this.playSound(PFSounds.TORVOSAURUS_BITE.get(), 1.0F, Torvosaurus.this.getVoicePitch());
				...
				if (this.getTicksUntilNextAttack() <= 10) {
					Torvosaurus.this.playWarningSound();
				}
			} else {
				this.resetAttackCooldown();
			}
	...
	public AgeableMob getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
		Torvosaurus entity = new Torvosaurus(PFEntities.TORVOSAURUS.get(), this.level());
	...
	public ItemStack getPickedResult(HitResult target) {
		return new ItemStack(PFItems.TORVOSAURUS_SPAWN_EGG.get());
	}
	
	public Item getEggItem() {
		return PFItems.TORVOSAURUS_EGG.get();
	}
    
	public BlockState getEggBlock(Level world, BlockPos pos) {
		return PFBlocks.TORVOSAURUS_EGG.get().defaultBlockState().setValue(DinosaurEggBlock.EGGS, Integer.valueOf(this.random.nextInt(4) + 1));
	}
