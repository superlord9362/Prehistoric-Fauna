...
public class StegosaurusSkeleton extends PrehistoricEntity {
	private static final EntityDataAccessor<Boolean> RESTING = SynchedEntityData.defineId(StegosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> RETRO = SynchedEntityData.defineId(StegosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> ACTION_LEFT = SynchedEntityData.defineId(StegosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> ACTION_RIGHT = SynchedEntityData.defineId(StegosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> PUSHING = SynchedEntityData.defineId(StegosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> LOOKING = SynchedEntityData.defineId(StegosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
	    ...
	    if (itemstack.getItem() == PFItems.GEOLOGY_HAMMER.get()) {
	    	if (!this.isResting() && !player.isShiftKeyDown() && !this.isClassical() && !this.isActionLeft() && !this.isActionRight()) {
	    		this.setResting(true);
	    	} else if (this.isResting() && !player.isShiftKeyDown()) {
	    		this.setResting(false);
	    		this.setClassical(true);
	    	} else if (!player.isShiftKeyDown() && this.isClassical()) {
	    		this.setClassical(false);
	    		this.setActionLeft(true);
	    	} else if (!player.isShiftKeyDown() && this.isActionLeft()) {
	    		this.setActionLeft(false);
	    		this.setActionRight(true);
	    	} else if (!player.isShiftKeyDown() && this.isActionRight()) {
	...
	private void spawnFossil(DamageSource p_213815_1_) {
	      Block.popResource(this.level, this.blockPosition(), new ItemStack(PFItems.STEGOSAURUS_SKELETON.get()));
	}
	
	static class LookAtPlayerGoal extends net.minecraft.world.entity.ai.goal.LookAtPlayerGoal {

		StegosaurusSkeleton entity;
		
		public LookAtPlayerGoal(StegosaurusSkeleton entityIn, Class<? extends LivingEntity> watchTargetClass, float maxDistance) {
