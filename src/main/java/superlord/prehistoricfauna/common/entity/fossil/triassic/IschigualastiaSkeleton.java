...
public class IschigualastiaSkeleton extends PrehistoricEntity {
	private static final EntityDataAccessor<Boolean> STRETCHING = SynchedEntityData.defineId(IschigualastiaSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> PUSHING = SynchedEntityData.defineId(IschigualastiaSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> LOOKING = SynchedEntityData.defineId(IschigualastiaSkeleton.class, EntityDataSerializers.BOOLEAN);
	...
	public boolean isStretching() {
		return this.entityData.get(STRETCHING);
	}

	private void setStretching(boolean isStretching) {
		this.entityData.set(STRETCHING, isStretching);
	}
	
	protected void defineSynchedData() {
		super.defineSynchedData();
		this.entityData.define(STRETCHING, false);
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
	    ...
	    if (itemstack.getItem() == PFItems.GEOLOGY_HAMMER.get()) {
	    	if (!this.isStretching() && !player.isShiftKeyDown()) {
	    		this.setStretching(true);
	    	} else if (this.isStretching() && !player.isShiftKeyDown()) {
	...
	private void spawnFossil(DamageSource p_213815_1_) {
	      Block.popResource(this.level, this.blockPosition(), new ItemStack(PFItems.ISCHIGUALASTIA_SKELETON.get()));
	}
	
	static class LookAtPlayerGoal extends net.minecraft.world.entity.ai.goal.LookAtPlayerGoal {

		IschigualastiaSkeleton entity;
		
		public LookAtPlayerGoal(IschigualastiaSkeleton entityIn, Class<? extends LivingEntity> watchTargetClass, float maxDistance) {
	...
	public ItemStack getPickedResult(HitResult target) {
		return new ItemStack(PFItems.ISCHIGUALASTIA_SKELETON.get());
	}
