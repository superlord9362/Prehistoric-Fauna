...
public class AllosaurusSkeleton extends PrehistoricEntity {
	private static final EntityDataAccessor<Boolean> ACTION_LEFT_POSE = SynchedEntityData.defineId(AllosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> ACTION_RIGHT_POSE = SynchedEntityData.defineId(AllosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> RESTING_POSE = SynchedEntityData.defineId(AllosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> RETRO_POSE = SynchedEntityData.defineId(AllosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> PUSHING = SynchedEntityData.defineId(AllosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> LOOKING = SynchedEntityData.defineId(AllosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	...
	public boolean isClassical() {
		return this.entityData.get(RETRO_POSE);
	}

	private void setClassical(boolean isRetro) {
		this.entityData.set(RETRO_POSE, isRetro);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(RETRO_POSE, false);
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
	    ...
	    if (itemstack.getItem() == PFItems.GEOLOGY_HAMMER.get()) {
	    	if (!this.isResting() && !this.isActionLeft() && !this.isActionRight() && !this.isClassical() && !player.isShiftKeyDown()) {
	...
	private void spawnFossil(DamageSource p_213815_1_) {
	      Block.popResource(this.level, this.blockPosition(), new ItemStack(PFItems.ALLOSAURUS_SKELETON.get()));
	}
	...
	public ItemStack getPickedResult(HitResult target) {
		return new ItemStack(PFItems.ALLOSAURUS_SKELETON.get());
	}
