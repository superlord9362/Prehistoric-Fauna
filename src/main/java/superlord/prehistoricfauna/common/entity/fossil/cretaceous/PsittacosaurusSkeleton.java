...
public class PsittacosaurusSkeleton extends PrehistoricEntity {
	private static final EntityDataAccessor<Boolean> LEFT = SynchedEntityData.defineId(PsittacosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> MATING = SynchedEntityData.defineId(PsittacosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> SAUNTER = SynchedEntityData.defineId(PsittacosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> SITTING = SynchedEntityData.defineId(PsittacosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> SPRINTING = SynchedEntityData.defineId(PsittacosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> PUSHING = SynchedEntityData.defineId(PsittacosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> LOOKING = SynchedEntityData.defineId(PsittacosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	...
	private void setLeft(boolean isLeft) {
		this.entityData.set(LEFT, isLeft);
	}
	
	public boolean isMating() {
		return this.entityData.get(MATING);
	}
	
	private void setMating(boolean isMating) {
		this.entityData.set(MATING, isMating);
	}
	...
	private void setSauntering(boolean isSauntering) {
		this.entityData.set(SAUNTER, isSauntering);
	}
	...
	private void setRunning(boolean isRunning) {
		this.entityData.set(SPRINTING, isRunning);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(MATING, false);
	...
	public void addAdditionalSaveData(CompoundTag compound) {
		...
		compound.putBoolean("IsLeft", this.isLeft());
		compound.putBoolean("IsMating", this.isMating());
		compound.putBoolean("IsSitting", this.isSitting());
		compound.putBoolean("IsSauntering", this.isSauntering());
		compound.putBoolean("IsSprinting", this.isRunning());
	...
	public void readAdditionalSaveData(CompoundTag compound) {
		...
		this.setLeft(compound.getBoolean("IsLeft"));
		this.setMating(compound.getBoolean("IsMating"));
		this.setSitting(compound.getBoolean("IsSitting"));
		this.setSauntering(compound.getBoolean("IsSauntering"));
		this.setRunning(compound.getBoolean("IsSprinting"));
	...
	public PsittacosaurusSkeleton(EntityType<? extends PsittacosaurusSkeleton> type, Level worldIn) {
		super(type, worldIn);
	}
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
	    ...
	    if (itemstack.getItem() == PFItems.GEOLOGY_HAMMER.get()) {
	    	if (!this.isLeft() && !this.isSitting() && !this.isMating() && !this.isSauntering() && !this.isRunning() && !player.isShiftKeyDown()) {
				this.setLeft(true);
			} else if (this.isLeft() && !player.isShiftKeyDown()) {
				this.setLeft(false);
				...
				this.setMating(true);
			} else if (this.isMating() && !player.isShiftKeyDown()) {
	...
	static class LookAtPlayerGoal extends net.minecraft.world.entity.ai.goal.LookAtPlayerGoal {
		...
		PsittacosaurusSkeleton entity;
		
		public LookAtPlayerGoal(PsittacosaurusSkeleton entityIn, Class<? extends LivingEntity> watchTargetClass, float maxDistance) {
	...
	public ItemStack getPickedResult(HitResult target) {
		return new ItemStack(PFItems.PSITTACOSAURUS_SKELETON.get());
	}
