...
public class DilophosaurusSkeleton extends PrehistoricEntity {
	private static final EntityDataAccessor<Boolean> STRUT = SynchedEntityData.defineId(DilophosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> GAZE = SynchedEntityData.defineId(DilophosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> ATTACK = SynchedEntityData.defineId(DilophosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> SITTING = SynchedEntityData.defineId(DilophosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> PUSHING = SynchedEntityData.defineId(DilophosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> LOOKING = SynchedEntityData.defineId(DilophosaurusSkeleton.class, EntityDataSerializers.BOOLEAN);
	...
	public boolean isStrutting() {
		return this.entityData.get(STRUT);
	}

	private void setStrutting(boolean isStrutting) {
		this.entityData.set(STRUT, isStrutting);
	}
	...
	private void setGazing(boolean isGazing) {
		this.entityData.set(GAZE, isGazing);
	}
	...
	private void setAttacking(boolean isAttacking) {
		this.entityData.set(ATTACK, isAttacking);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(STRUT, false);
	...
	public DilophosaurusSkeleton(EntityType<? extends DilophosaurusSkeleton> type, Level worldIn) {
		super(type, worldIn);
	}
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
	    ...
	    if (itemstack.getItem() == PFItems.GEOLOGY_HAMMER.get()) {
	    	if (!this.isStrutting() && !this.isGazing() && !this.isAttacking() && !this.isSitting() && !player.isShiftKeyDown()) {
				this.setStrutting(true);
			} else if (this.isStrutting() && !player.isShiftKeyDown()) {
				this.setStrutting(false);
				this.setGazing(true);
			} else if (this.isGazing() && !player.isShiftKeyDown()) {
				this.setGazing(false);
				this.setAttacking(true);
			} else if (this.isAttacking() && !player.isShiftKeyDown()) {
	...
	private void spawnFossil(DamageSource p_213815_1_) {
	      Block.popResource(this.level, this.blockPosition(), new ItemStack(PFItems.DILOPHOSAURUS_SKELETON.get()));
	}
	
	static class LookAtPlayerGoal extends net.minecraft.world.entity.ai.goal.LookAtPlayerGoal {

		DilophosaurusSkeleton entity;
		
		public LookAtPlayerGoal(DilophosaurusSkeleton entityIn, Class<? extends LivingEntity> watchTargetClass, float maxDistance) {
