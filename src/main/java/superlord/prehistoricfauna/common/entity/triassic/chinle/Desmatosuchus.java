...
public class Desmatosuchus extends DinosaurEntity {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new DinosaurWaterAvoidingRandomStrollGoal(this, 1.0D));
		...
		this.goalSelector.addGoal(8, new AvoidEntityGoal<LivingEntity>(this, LivingEntity.class, 7F, 1.5D, 1.75D, (p_213487_0_) -> {
			return p_213487_0_.getType().is(PFTags.DESMATOSUCHUS_AVOIDING);
		}));
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if (item instanceof PaleopediaItem) {
			if (!itemstack.getTag().contains("Pages", EnumPaleoPages.DESMATOSUCHUS.ordinal())) {
				EnumPaleoPages.addPage(EnumPaleoPages.fromInt(EnumPaleoPages.DESMATOSUCHUS.ordinal()), itemstack);
				player.displayClientMessage(Component.translatable("paleopedia.desmatosuchus_added"), true);
	...
	public BlockState getEggBlock(Level world, BlockPos pos) {
		return PFBlocks.DESMATOSUCHUS_NEST.get().defaultBlockState().setValue(NestAndEggsBlock.EGGS, Integer.valueOf(this.random.nextInt(4) + 1)).setValue(NestAndEggsBlock.PLANT_LEVEL, Integer.valueOf(this.random.nextInt(3) + 1));
	}
