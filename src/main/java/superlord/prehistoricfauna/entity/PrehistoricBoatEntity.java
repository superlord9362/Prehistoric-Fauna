...
public class PrehistoricBoatEntity extends Entity {
   private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(PrehistoricBoatEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(PrehistoricBoatEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(PrehistoricBoatEntity.class, DataSerializers.FLOAT);
   private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(PrehistoricBoatEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(PrehistoricBoatEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(PrehistoricBoatEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(PrehistoricBoatEntity.class, DataSerializers.VARINT);
   ...
   private PrehistoricBoatEntity.Status status;
   private PrehistoricBoatEntity.Status previousStatus;
...
public PrehistoricBoatEntity(EntityType boat, World p_i50129_2_) {
   ...
   protected void registerData() {
      ...
      this.dataManager.register(BOAT_TYPE, PrehistoricBoatEntity.Type.ARAUCARIA.ordinal());
...
public boolean canBeCollidedWith() {
   ...
   public void tick() {
      ...
      if (this.status != PrehistoricBoatEntity.Status.UNDER_WATER && this.status != PrehistoricBoatEntity.Status.UNDER_FLOWING_WATER) {
   ...
   private PrehistoricBoatEntity.Status getBoatStatus() {
      ...
      if (boatentity$status != null) {
         ...
         return PrehistoricBoatEntity.Status.IN_WATER;
      } else {
         float f = this.getBoatGlide();
         if (f > 0.0F) {
            this.boatGlide = f;
            return PrehistoricBoatEntity.Status.ON_LAND;
         } else {
            return PrehistoricBoatEntity.Status.IN_AIR;
         }
      }
   }
               ...
               for(int i2 = i1; i2 < j1; ++i2) {
                  ...
                  if (ifluidstate.isTagged(FluidTags.WATER) && d0 < (double)((float)blockpos$pooledmutable.getY() + ifluidstate.getActualHeight(this.world, blockpos$pooledmutable))) {
                     if (!ifluidstate.isSource()) {
                        PrehistoricBoatEntity.Status boatentity$status = PrehistoricBoatEntity.Status.UNDER_FLOWING_WATER;
                        return boatentity$status;
                     }
   ...
   private void updateMotion() {
	...
	double d0 = (double)-0.04F;
      ...
      double d2 = 0.0D;
      this.momentum = 0.05F;
      if (this.previousStatus == PrehistoricBoatEntity.Status.IN_AIR && this.status != PrehistoricBoatEntity.Status.IN_AIR && this.status != PrehistoricBoatEntity.Status.ON_LAND) {
         ...
         this.lastYd = 0.0D;
         this.status = PrehistoricBoatEntity.Status.IN_WATER;
      } else {
         if (this.status == PrehistoricBoatEntity.Status.IN_WATER) {
            d2 = (this.waterLevel - this.getPosY()) / (double)this.getHeight();
            this.momentum = 0.9F;
         } else if (this.status == PrehistoricBoatEntity.Status.UNDER_FLOWING_WATER) {
            d1 = -7.0E-4D;
            this.momentum = 0.9F;
         } else if (this.status == PrehistoricBoatEntity.Status.UNDER_WATER) {
            d2 = (double)0.01F;
            this.momentum = 0.45F;
         } else if (this.status == PrehistoricBoatEntity.Status.IN_AIR) {
            this.momentum = 0.9F;
         } else if (this.status == PrehistoricBoatEntity.Status.ON_LAND) {
...
protected void updateFallState(double y, boolean onGroundIn, BlockState state, BlockPos pos) {
      ...
      if (!this.isPassenger()) {
         if (onGroundIn) {
            if (this.fallDistance > 3.0F) {
               if (this.status != PrehistoricBoatEntity.Status.ON_LAND) {
   ...
   public void setBoatType(PrehistoricBoatEntity.Type boatType) {
      this.dataManager.set(BOAT_TYPE, boatType.ordinal());
   }

   public PrehistoricBoatEntity.Type getBoatType() {
      return PrehistoricBoatEntity.Type.byId(this.dataManager.get(BOAT_TYPE));
   }
   ...
   public static enum Type {
      ARAUCARIA(BlockInit.ARAUCARIA_PLANKS, "araucaria"),
      METASEQUOIA(BlockInit.METASEQUOIA_PLANKS, "metasequoia");
      ...
      public static PrehistoricBoatEntity.Type getTypeFromString(String nameIn) {
         PrehistoricBoatEntity.Type[] aboatentity$type = values();
