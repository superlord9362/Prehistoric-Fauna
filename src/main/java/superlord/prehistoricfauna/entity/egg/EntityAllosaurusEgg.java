...
public class EntityAllosaurusEgg extends EntityThrowable {
    public EntityAllosaurusEgg(World worldIn) {
        super(worldIn);
    }

    public EntityAllosaurusEgg(World worldIn, EntityLivingBase throwerIn) {
        super(worldIn, throwerIn);
    }

    public EntityAllosaurusEgg(World worldIn, double x, double y, double z) {
        super(worldIn, x, y, z);
    }
    ...
    protected void onImpact(RayTraceResult result) {
        ...
        if (!this.world.isRemote) {
            if (this.rand.nextInt(8) == 0) {
                ...
                for (int j = 0; j < i; ++j) {
                    EntityAllosaurus entitychicken = new EntityAllosaurus(this.world);
