...
public class EntityPrenocephaleEgg extends EntityThrowable {
    public EntityPrenocephaleEgg(World worldIn) {
        super(worldIn);
    }

    public EntityPrenocephaleEgg(World worldIn, EntityLivingBase throwerIn) {
        super(worldIn, throwerIn);
    }

    public EntityPrenocephaleEgg(World worldIn, double x, double y, double z) {
        super(worldIn, x, y, z);
    }
    ...
    protected void onImpact(RayTraceResult result) {
        ...
        if (!this.world.isRemote) {
            if (this.rand.nextInt(8) == 0) {
                ...
                for (int j = 0; j < i; ++j) {
                	EntityPrenocephale entitychicken = new EntityPrenocephale(this.world);
