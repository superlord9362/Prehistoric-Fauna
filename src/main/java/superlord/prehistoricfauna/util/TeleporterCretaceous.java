...
public class TeleporterCretaceous implements ITeleporter {
    ...
    public BlockPattern.PortalInfo placeInExistingPortal(ServerWorld world, BlockPos pos, Vec3d motion, Direction direction, double x, double y, boolean isPlayer) {
        ...
        if (!isPlayer && this.columnMap.containsKey(columnpos)) {
        	...
        	TeleporterCretaceous.PortalPosition position = this.destinationCoordinateCache.get(columnpos);
            if (position != null) {
                ...
                for(int eX = -128; eX <= 128; ++eX) {
                    ...
                    for(int eZ = -128; eZ <= 128; ++eZ) {
                        for(BlockPos blockpos1 = pos.add(eX, world.getActualHeight() - 1 - pos.getY(), eZ); blockpos1.getY() >= 0; blockpos1 = blockpos2) {
                            ...
                            if (world.getBlockState(blockpos1).getBlock() == BlockInit.CRETACEOUS_PORTAL.get()) {
                                for(blockpos2 = blockpos1.down(); world.getBlockState(blockpos2).getBlock() == BlockInit.CRETACEOUS_PORTAL.get(); blockpos2 = blockpos2.down()) {
                                    blockpos1 = blockpos2;
                                }
            ...
            if (blockpos == null) {
                ...
                if (isFrame) {
                    this.destinationCoordinateCache.put(columnpos, new TeleporterCretaceous.PortalPosition(blockpos, world.getGameTime()));
                ...
                BlockPattern.PatternHelper helper = CretaceousPortalBlock.createPatternHelper(world, blockpos);
    ...
    public void makePortal(ServerWorld world, Entity entity) {
        ...
        //Portal blocks
        world.setBlockState(pos.up(), BlockInit.CRETACEOUS_PORTAL.get().getDefaultState());
		world.setBlockState(pos.up(2), BlockInit.CRETACEOUS_PORTAL.get().getDefaultState());
		world.setBlockState(pos.up(3), BlockInit.CRETACEOUS_PORTAL.get().getDefaultState());
		world.setBlockState(pos.east().up(), BlockInit.CRETACEOUS_PORTAL.get().getDefaultState());
		world.setBlockState(pos.east().up(2), BlockInit.CRETACEOUS_PORTAL.get().getDefaultState());
		world.setBlockState(pos.east().up(3), BlockInit.CRETACEOUS_PORTAL.get().getDefaultState());
		world.setBlockState(pos.west().up(), BlockInit.CRETACEOUS_PORTAL.get().getDefaultState());
		world.setBlockState(pos.west().up(2), BlockInit.CRETACEOUS_PORTAL.get().getDefaultState());
		world.setBlockState(pos.west().up(3), BlockInit.CRETACEOUS_PORTAL.get().getDefaultState());
