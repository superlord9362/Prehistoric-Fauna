        ...
        for (int i1 = 0; i1 < 3; ++i1) {
            ...
            if (l1 == -1) {
                ...
                if (PrehistoricFauna.PROXY.getReferencedTE() instanceof PaleoscribeTileEntity) {
                    ...
                    if (container.getSlot(0).getStack().getItem() == PFItems.PALEOPEDIA.get()) { // Forge: render buttons as disabled when enchantable but enchantability not met on lower levels
                        int k2 = mouseX - (i + 60);
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            if (this.isPointInRegion(60, 14 + 19 * j, 108, 17, mouseX, mouseY) && k > 0) {
                ...
                if (enchantment == null) {
                    list.add(new StringTextComponent(TextFormatting.RED + I18n.format("container.paleoscribe.no_paleopedia")).func_241878_f());
                } else if (!flag) {
                    list.add(new StringTextComponent("" + TextFormatting.WHITE + TextFormatting.ITALIC + I18n.format(enchantment == null ? "" : "paleopedia." + enchantment.name().toLowerCase())).func_241878_f());
                    TextFormatting textformatting = i >= i1 ? TextFormatting.GRAY : TextFormatting.RED;
                    list.add(new StringTextComponent(textformatting + "" + I18n.format("container.paleoscribe.costs")).func_241878_f());
