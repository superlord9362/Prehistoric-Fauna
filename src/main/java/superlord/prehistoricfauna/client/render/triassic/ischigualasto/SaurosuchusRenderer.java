...
public class SaurosuchusRenderer extends MobRenderer<SaurosuchusEntity, EntityModel<SaurosuchusEntity>> {
	...
	public ResourceLocation getEntityTexture(SaurosuchusEntity entity) {
		if (entity.isAlbino()) {
			return ALBINO;
		} else if (entity.isAlbino() && (entity.isSleeping() || entity.isAsleep())) {
			return ALBINO_SLEEPING;
		} else if (entity.isMelanistic()) {
			return MELANISTIC;
		} else if (entity.isMelanistic() && (entity.isSleeping() || entity.isAsleep())) {
			return MELANISTIC_SLEEPING;
		} else if((entity.isSleeping() || entity.isAsleep()) && !entity.isMelanistic() && !entity.isAlbino()) {
			return SLEEPING;
		} else {
			return SAUROSUCHUS;
		}
