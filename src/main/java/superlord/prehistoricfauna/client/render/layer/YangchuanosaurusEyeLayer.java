...
public class YangchuanosaurusEyeLayer extends RenderLayer<Yangchuanosaurus, EntityModel<Yangchuanosaurus>> {
	
	private static final RenderType TEXTURE = RenderType.eyes(new ResourceLocation(PrehistoricFauna.MOD_ID, "textures/entities/yangchuanosaurus/yangchuanosaurus_eyeglow.png"));
	private final RenderLayerParent<Yangchuanosaurus, EntityModel<Yangchuanosaurus>> yangchuanosaurusRenderer;
	
	public YangchuanosaurusEyeLayer(RenderLayerParent<Yangchuanosaurus, EntityModel<Yangchuanosaurus>> rendererIn) {
		super(rendererIn);
	...
	public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, Yangchuanosaurus yangchuanosaurus, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (!(yangchuanosaurusRenderer.getModel() instanceof YangchuanosaurusModel)) {
			return;
		}
