...
public class TyrannosaurusEyeLayer extends LayerRenderer<TyrannosaurusEntity, EntityModel<TyrannosaurusEntity>> {
	
	private static final RenderType TEXTURE = RenderType.getEyes(new ResourceLocation(PrehistoricFauna.MOD_ID, "textures/entities/tyrannosaurus/tyrannosaurusrex_eyeglow.png"));

	private final IEntityRenderer<TyrannosaurusEntity, EntityModel<TyrannosaurusEntity>> tyrannosaurusRenderer;
	
	public TyrannosaurusEyeLayer(IEntityRenderer<TyrannosaurusEntity, EntityModel<TyrannosaurusEntity>> tyrannosaurusRenderer) {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, TyrannosaurusEntity tyrannosaurus, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (!(tyrannosaurusRenderer.getEntityModel() instanceof TyrannosaurusrexModel)) {
			return;
		}
