...
public class DidelphodonEyeLayer extends RenderLayer<Didelphodon, DidelphodonModel> {
	...
	@Override
	public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, Didelphodon dakotaraptor, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (!(dakotaraptorRenderer.getModel() instanceof DidelphodonModel)) {
			return;
		}
