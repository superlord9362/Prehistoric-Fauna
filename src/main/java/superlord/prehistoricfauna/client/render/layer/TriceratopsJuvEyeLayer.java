...
public class TriceratopsJuvEyeLayer extends LayerRenderer<TriceratopsEntity, EntityModel<TriceratopsEntity>> {
	
	private static final RenderType TEXTURE = RenderType.getEyes(new ResourceLocation(PrehistoricFauna.MOD_ID, "textures/entities/triceratops/triceratopsjuv_eyeglow.png"));
	private final IEntityRenderer<TriceratopsEntity, EntityModel<TriceratopsEntity>> triceratopsRenderer;
	
	public TriceratopsJuvEyeLayer(IEntityRenderer<TriceratopsEntity, EntityModel<TriceratopsEntity>> triceratopsRenderer) {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, TriceratopsEntity triceratops, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (!(triceratopsRenderer.getEntityModel() instanceof TriceratopsJuvenileModel)) {
			return;
		}
