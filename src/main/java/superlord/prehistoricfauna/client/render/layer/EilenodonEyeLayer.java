...
public class EilenodonEyeLayer extends LayerRenderer<EilenodonEntity, EntityModel<EilenodonEntity>> {
	...
	public EilenodonEyeLayer(IEntityRenderer<EilenodonEntity, EntityModel<EilenodonEntity>> eilenodonRenderer) {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, EilenodonEntity eilenodon, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (!(eilenodonRenderer.getEntityModel() instanceof EilenodonModel)) {
			return;
		}
