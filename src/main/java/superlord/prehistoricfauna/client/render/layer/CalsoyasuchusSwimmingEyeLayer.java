...
public class CalsoyasuchusSwimmingEyeLayer extends LayerRenderer<CalsoyasuchusEntity, EntityModel<CalsoyasuchusEntity>> {
	...
	public CalsoyasuchusSwimmingEyeLayer(IEntityRenderer<CalsoyasuchusEntity, EntityModel<CalsoyasuchusEntity>> calsoyasuchusRenderer) {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, CalsoyasuchusEntity calsoyasuchus, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (!(calsoyasuchusRenderer.getEntityModel() instanceof CalsoyasuchusSwimmingModel)) {
			return;
		}
