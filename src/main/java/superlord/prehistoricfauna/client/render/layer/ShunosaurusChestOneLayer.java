...
public class ShunosaurusChestOneLayer extends RenderLayer<Shunosaurus, EntityModel<Shunosaurus>> {
	
	private static final RenderType TEXTURE = RenderType.entityCutoutNoCull(new ResourceLocation(PrehistoricFauna.MOD_ID, "textures/entity/shunosaurus/shunosaurus_chest_1.png"));
	private final RenderLayerParent<Shunosaurus, EntityModel<Shunosaurus>> shunosaurusRenderer;
	
	public ShunosaurusChestOneLayer(RenderLayerParent<Shunosaurus, EntityModel<Shunosaurus>> shunosaurusRenderer) {
	...
	public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, Shunosaurus shunosaurus, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (shunosaurus.getChestCount() >= 1) {
			RenderType tex = null;
			tex = TEXTURE;
	        if(tex != null){
