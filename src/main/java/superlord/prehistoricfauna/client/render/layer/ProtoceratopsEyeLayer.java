...
public class ProtoceratopsEyeLayer extends LayerRenderer<ProtoceratopsEntity, EntityModel<ProtoceratopsEntity>> {
	...
	public ProtoceratopsEyeLayer(IEntityRenderer<ProtoceratopsEntity, EntityModel<ProtoceratopsEntity>> protoceratopsRenderer) {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, ProtoceratopsEntity protoceratops, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (!(protoceratopsRenderer.getEntityModel() instanceof ProtoceratopsModel)) {
			return;
		}
