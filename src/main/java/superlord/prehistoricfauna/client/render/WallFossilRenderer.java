...
public class WallFossilRenderer extends EntityRenderer<WallFossil> {
    private static final ResourceLocation BACK = new ResourceLocation(PrehistoricFauna.MOD_ID, "textures/painting/back.png");
    private static final Map<WallFossil.WallFossilType, ResourceLocation> WALL_FOSSIL = new HashMap<>();

    public WallFossilRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager);
    }

    public void render(WallFossil entityIn, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int packedLightIn) {
        matrixStack.pushPose();
        matrixStack.mulPose(Vector3f.YP.rotationDegrees(180.0F - entityYaw));
        WallFossil.WallFossilType painting = entityIn.wallfossil;
        float f = 0.0625F;
    ...
    @Override
    public ResourceLocation getTextureLocation(WallFossil entity) {
        return WALL_FOSSIL.computeIfAbsent(entity.wallfossil, k -> new ResourceLocation(PrehistoricFauna.MOD_ID, "textures/fossils/" + k.name().toLowerCase() + ".png"));
    }

    private void renderPainting(PoseStack p_229122_1_, MultiBufferSource vertexProvider, WallFossil painting, int width, int height) {
