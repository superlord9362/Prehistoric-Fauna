 ...
 */
@OnlyIn(Dist.CLIENT)
public class BasilemysModel extends EntityModel<BasilemysEntity> {
	...
	public void setRotationAngles(BasilemysEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (entityIn.isPanicing() || entityIn.isAsleep()) {
			...
			if (entityIn.isEating()) {
				this.Neck.rotateAngleX = Math.abs(MathHelper.sin(0.05F * ageInTicks) * 0.25F);
			}
		}
	}
