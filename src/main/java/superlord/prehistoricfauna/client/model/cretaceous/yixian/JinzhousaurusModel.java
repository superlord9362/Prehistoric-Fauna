			...
			if (sleepProgress != 0 && entity.getEntityData().get(DinosaurEntity.SLEEP_TICK) > 0) {
				...
				if (entity.isBipedal()) {
					if (bipedalProgress != 0 && entity.getEntityData().get(Jinzhousaurus.BIPEDAL_TICK) > 0) {
						this.Hips.xRot = Mth.lerp(bipedalProgress, 0, -0.3F);
						this.Chest.xRot = Mth.lerp(bipedalProgress, 0, 0.15F);
						this.LeftArm.xRot = Mth.lerp(bipedalProgress, 0, 0.5F);
						this.RightArm.xRot = Mth.lerp(bipedalProgress, 0, 0.5F);
						this.Tail1.xRot = Mth.lerp(bipedalProgress, (-Math.abs(-0.05F * Mth.sin(0.1F * ageInTicks / 5))) + (Mth.cos(limbSwing * speed * 0.3F) * degree * 0.15F * limbSwingAmount - 0.15F), 0.05F);
					} else {
						crouchPose();
						this.Tail1.xRot = (-Math.abs(-0.05F * Mth.sin(0.1F * ageInTicks / 5))) + (Mth.cos(limbSwing * speed * 0.3F) * degree * 0.15F * limbSwingAmount + 0.05F);
					}
					...
					if (bipedalProgress != 0 && entity.getEntityData().get(Jinzhousaurus.BIPEDAL_TICK) > 0) {
						this.Hips.xRot = Mth.lerp(bipedalProgress, -0.3F, 0);
						this.Chest.xRot = Mth.lerp(bipedalProgress, 0.15F, 0);
						this.LeftArm.xRot = Mth.lerp(bipedalProgress, 0.5F, 0);
						this.RightArm.xRot = Mth.lerp(bipedalProgress, 0.5F, 0);
						this.Tail1.xRot = Mth.lerp(bipedalProgress, 0.05F, (-Math.abs(-0.05F * Mth.sin(0.1F * ageInTicks / 5))) + (Mth.cos(limbSwing * speed * 0.3F) * degree * 0.15F * limbSwingAmount - 0.15F));
						this.RightThigh.xRot = Mth.cos(1.0F + limbSwing * speed * 0.3F) * degree * 0.4F * limbSwingAmount + attackProgress * (float) Math.toRadians(7F);
						this.RightLeg.xRot = Mth.cos(limbSwing * speed * 0.3F) * degree * 0.3F * limbSwingAmount;
						this.RightFoot.xRot = Mth.cos(limbSwing * speed * 0.3F) * degree * -0.3F * limbSwingAmount;
						this.LeftThigh.xRot = Mth.cos(1.0F + limbSwing * speed * 0.3F) * degree * -0.4F * limbSwingAmount + attackProgress * (float) Math.toRadians(7F);
