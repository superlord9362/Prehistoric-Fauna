	...
	public void setupAnim(Jinzhousaurus entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (entity.isAsleep()) {
			if (sleepProgress != 0 && entity.getEntityData().get(DinosaurEntity.SLEEP_TICK) > 0) {
				...
				this.RightThigh.y = Mth.lerp(sleepProgress, -27.5F, -14);
				this.LeftThigh.y = Mth.lerp(sleepProgress, -27.5F, -14F);
			...
			if (sleepProgress != 0 && entity.getEntityData().get(DinosaurEntity.SLEEP_TICK) > 0) {
				this.Hips.y = Mth.lerp(sleepProgress, -3F, -14.5F);
				this.RightThigh.y = Mth.lerp(sleepProgress, -14F, -27.5F);
				this.LeftThigh.y = Mth.lerp(sleepProgress, -14F, -27.5F);
