	...
	public void setupAnim(Edmontosaurus entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (entity.isAsleep()) {
			if (sleepProgress != 0 && entity.getEntityData().get(DinosaurEntity.SLEEP_TICK) > 0) {
				this.Body.y = Mth.lerp(sleepProgress, -7.5F, -4.5F);
				this.LeftThigh.y = Mth.lerp(sleepProgress, -9.5F, -7F);
				this.LeftThigh.z = Mth.lerp(sleepProgress, -3, -2.5F);
				this.RightThigh.y = Mth.lerp(sleepProgress, -9.5F, -7F);
				this.RightThigh.z = Mth.lerp(sleepProgress, -3, -2.5F);
				this.LeftArm.y = Mth.lerp(sleepProgress, 1, -0.5F);
				this.RightArm.y = Mth.lerp(sleepProgress, 1, -0.5F);
				this.Tail.z = Mth.lerp(sleepProgress, 2, 1F);
				this.Neck.xRot = Mth.lerp(sleepProgress, 0, 0.1235987755982988F);
			...
			if (sleepProgress != 0 && entity.getEntityData().get(DinosaurEntity.SLEEP_TICK) > 0) {
				this.Body.y = Mth.lerp(sleepProgress, -4.5F, -7.5F);
				this.LeftThigh.y = Mth.lerp(sleepProgress, -7F, -9.5F);
				this.LeftThigh.z = Mth.lerp(sleepProgress, -2.5F, -3F);
				this.RightThigh.y = Mth.lerp(sleepProgress, -7F, -9.5F);
				this.RightThigh.z = Mth.lerp(sleepProgress, -2.5F, -3F);
				this.LeftArm.y = Mth.lerp(sleepProgress, -0.5F, 1F);
				this.RightArm.y = Mth.lerp(sleepProgress, -0.5F, 1F);
				this.Tail.z = Mth.lerp(sleepProgress, 1, 2F);
				this.Neck.xRot = Mth.lerp(sleepProgress, 0.1235987755982988F, 0);
