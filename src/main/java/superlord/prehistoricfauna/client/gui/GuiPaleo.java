	...
	protected void renderBg(@NotNull PoseStack matrixStack, float partialTicks, int mouseX, int mouseY) {
		...
		for (int i1 = 0; i1 < 3; ++i1) {
			...
			if (l1 == -1) {
				...
				if (enchantment != null) {
					s1 = I18n.get("paleopedia." + enchantment.toString().toLowerCase());//EnchantmentNameParts.getInstance().generateNewRandomName(this.fontRenderer, l1);
				...
				if (PrehistoricFauna.PROXY.getReferencedBE() instanceof PaleoscribeBlockEntity) {
					...
					if (menu.getSlot(0).getItem().getItem() == PFItems.PALEOPEDIA.get()) { // Forge: render buttons as disabled when enchantable but enchantability not met on lower levels
						int k2 = mouseX - (i + 60);
