...
public class SillosuchusEggBlock extends Block {
   ...
   public SillosuchusEggBlock(Block.Properties properties) {
      super(properties);
      ...
      if (this.canGrow(worldIn) && this.hasProperHabitat(worldIn, pos)) {
         ...
         if (i < 2) {
            ...
            for(int j = 0; j < state.get(EGGS); ++j) {
               worldIn.playEvent(2001, pos, Block.getStateId(state));
               SillosuchusEntity sillosuchusentity = ModEntityTypes.SILLOSUCHUS_ENTITY.create(worldIn);
               sillosuchusentity.setGrowingAge(-24000);
               sillosuchusentity.setLocationAndAngles((double)pos.getX() + 0.3D + (double)j * 0.2D, (double)pos.getY(), (double)pos.getZ() + 0.3D, 0.0F, 0.0F);
               worldIn.addEntity(sillosuchusentity);
            }
         }
      }
   ...
   private boolean canTrample(World worldIn, Entity trampler) {
      if (trampler instanceof SillosuchusEntity) {
         return false;
      } else {
