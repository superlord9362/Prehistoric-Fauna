...
public class CamarasaurusEggBlock extends Block {
   ...
   public CamarasaurusEggBlock(Block.Properties properties) {
      super(properties);
      ...
      if (this.canGrow(worldIn) && this.hasProperHabitat(worldIn, pos)) {
         ...
         if (i < 2) {
            ...
            for(int j = 0; j < state.get(EGGS); ++j) {
               worldIn.playEvent(2001, pos, Block.getStateId(state));
               CamarasaurusEntity camarasaurusentity = ModEntityTypes.CAMARASAURUS_ENTITY.create(worldIn);
               camarasaurusentity.setGrowingAge(-24000);
               camarasaurusentity.setLocationAndAngles((double)pos.getX() + 0.3D + (double)j * 0.2D, (double)pos.getY(), (double)pos.getZ() + 0.3D, 0.0F, 0.0F);
               worldIn.addEntity(camarasaurusentity);
            }
         }
      }
   ...
   private boolean canTrample(World worldIn, Entity trampler) {
      if (trampler instanceof CamarasaurusEntity) {
         return false;
      } else {
