...
public class DakotaraptorEggBlock extends Block {
   ...
   public DakotaraptorEggBlock(Block.Properties properties) {
      super(properties);
      ...
      if (this.canGrow(worldIn) && this.hasProperHabitat(worldIn, pos)) {
         ...
         if (i < 2) {
            ...
            for(int j = 0; j < state.get(EGGS); ++j) {
               worldIn.playEvent(2001, pos, Block.getStateId(state));
               DakotaraptorEntity dakotaraptorentity = ModEntityTypes.DAKOTARAPTOR_ENTITY.create(worldIn);
               dakotaraptorentity.setGrowingAge(-24000);
               dakotaraptorentity.setLocationAndAngles((double)pos.getX() + 0.3D + (double)j * 0.2D, (double)pos.getY(), (double)pos.getZ() + 0.3D, 0.0F, 0.0F);
               worldIn.addEntity(dakotaraptorentity);
            }
         }
      }
...
public boolean isReplaceable(BlockState state, BlockItemUseContext useContext) {
   ...
   private boolean canTrample(World worldIn, Entity trampler) {
      if (trampler instanceof DakotaraptorEntity) {
         return false;
      } else {
