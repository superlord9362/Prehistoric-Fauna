...
public class TriassicPortalBlock extends BreakableBlock {
    ...
    public boolean tryToCreatePortal(World worldIn, BlockPos pos) {
        TriassicPortalBlock.Size portalSize = this.isPortal(worldIn, pos);
    ...
    public void animateTick(BlockState state, World world, BlockPos pos, Random rand) {
    	...
    	for (int i = 0; i < 4; i++) {
    		...
    		world.addParticle(ParticleInit.TRIASSIC.get(), x, y, z, sX, sY, sZ);
    	}
    }
        ...
        int calculatePortalHeight() {
            ...
            for (height = 0; height < 21; ++height) {
                for (int i = 0; i < width; ++i) {
                    ...
                    if (blockstate.getBlock() == BlockInit.TRIASSIC_PORTAL.get()) {
        ...
        void placePortalBlocks() {
            for (int i = 0; i < this.width; ++i) {
                ...
                for (int j = 0; j < height; ++j) {
                    world.setBlockState(blockpos.up(j), BlockInit.TRIASSIC_PORTAL.get().getDefaultState().with(AXIS, axis), 2);
                }
            }
        }
