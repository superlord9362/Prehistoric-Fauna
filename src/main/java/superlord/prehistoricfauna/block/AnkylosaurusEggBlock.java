...
public class AnkylosaurusEggBlock extends Block {
   ...
   public AnkylosaurusEggBlock(Block.Properties properties) {
      super(properties);
      ...
      if (this.canGrow(worldIn) && this.hasProperHabitat(worldIn, pos)) {
         ...
         if (i < 2) {
            ...
            for(int j = 0; j < state.get(EGGS); ++j) {
               worldIn.playEvent(2001, pos, Block.getStateId(state));
               AnkylosaurusEntity ankylosaurusentity = ModEntityTypes.ANKYLOSAURUS_ENTITY.create(worldIn);
               ankylosaurusentity.setGrowingAge(-24000);
               ankylosaurusentity.setLocationAndAngles((double)pos.getX() + 0.3D + (double)j * 0.2D, (double)pos.getY(), (double)pos.getZ() + 0.3D, 0.0F, 0.0F);
               worldIn.addEntity(ankylosaurusentity);
            }
         }
      }
   ...
   private boolean hasProperHabitat(IBlockReader p_203168_1_, BlockPos p_203168_2_) {
      return p_203168_1_.getBlockState(p_203168_2_.down()).getBlock() == Blocks.SAND || p_203168_1_.getBlockState(p_203168_2_.down()).getBlock() == Blocks.COARSE_DIRT;
   }
...
public boolean isReplaceable(BlockState state, BlockItemUseContext useContext) {
   ...
   private boolean canTrample(World worldIn, Entity trampler) {
      if (trampler instanceof AnkylosaurusEntity) {
         return false;
      } else {
