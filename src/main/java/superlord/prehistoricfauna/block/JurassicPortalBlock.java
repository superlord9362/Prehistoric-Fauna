...
public class JurassicPortalBlock extends BreakableBlock {
    ...
    public JurassicPortalBlock(Properties props) {
        super(props);
    ...
    @Nullable
    public JurassicPortalBlock.Size isPortal(IWorld world, BlockPos pos) {
        JurassicPortalBlock.Size gaiaPortalSizeX = new JurassicPortalBlock.Size(world, pos, Direction.Axis.X);
        if (gaiaPortalSizeX.isValid() && gaiaPortalSizeX.portalBlockCount == 0) {
            return gaiaPortalSizeX;
        } else {
            JurassicPortalBlock.Size gaiaPortalSizeZ = new JurassicPortalBlock.Size(world, pos, Direction.Axis.Z);
            ...
            if (entity.timeUntilPortal > 0) {
                ...
                if (entity.world instanceof ServerWorld) {
                    if (entity.world.getServer().getAllowNether() && !entity.isPassenger()) {
                        entity.timeUntilPortal = entity.getPortalCooldown();
                        DimensionType type = worldIn.dimension.getType() == DimensionTypeInit.JURASSIC_DIMENSION_TYPE ? DimensionType.OVERWORLD : DimensionTypeInit.JURASSIC_DIMENSION_TYPE;
                        entity.changeDimension(type, new TeleporterJurassic());
                    }
                }
            }
        }
	...
	public static BlockPattern.PatternHelper createPatternHelper(IWorld worldIn, BlockPos pos) {
        ...
        JurassicPortalBlock.Size size = new JurassicPortalBlock.Size(worldIn, pos, Direction.Axis.X);
        LoadingCache<BlockPos, CachedBlockInfo> cache = BlockPattern.createLoadingCache(worldIn, true);
        if (!size.isValid()) {
            axis = Direction.Axis.X;
            size = new JurassicPortalBlock.Size(worldIn, pos, Direction.Axis.Z);
        }
