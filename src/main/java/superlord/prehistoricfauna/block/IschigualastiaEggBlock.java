...
public class IschigualastiaEggBlock extends Block {
		...
		if (this.canGrow(worldIn) && this.hasProperHabitat(worldIn, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					worldIn.playEvent(2001, pos, Block.getStateId(state));
					IschigualastiaEntity ischigualastiaentity = ModEntityTypes.ISCHIGUALASTIA_ENTITY.create(worldIn);
					ischigualastiaentity.setGrowingAge(-24000);
					ischigualastiaentity.setLocationAndAngles((double) pos.getX() + 0.3D + (double) j * 0.2D,
							(double) pos.getY(), (double) pos.getZ() + 0.3D, 0.0F, 0.0F);
					worldIn.addEntity(ischigualastiaentity);
				}
			}
		}
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		if (trampler instanceof IschigualastiaEntity) {
			return false;
		} else {
