...
public class BasilemysEggBlock extends Block {
   ...
   public BasilemysEggBlock(Block.Properties properties) {
      super(properties);
      ...
      if (this.canGrow(worldIn) && this.hasProperHabitat(worldIn, pos)) {
         ...
         if (i < 2) {
            ...
            for(int j = 0; j < state.get(EGGS); ++j) {
               worldIn.playEvent(2001, pos, Block.getStateId(state));
               BasilemysEntity basilemysentity = ModEntityTypes.BASILEMYS_ENTITY.create(worldIn);
               basilemysentity.setGrowingAge(-24000);
               basilemysentity.setLocationAndAngles((double)pos.getX() + 0.3D + (double)j * 0.2D, (double)pos.getY(), (double)pos.getZ() + 0.3D, 0.0F, 0.0F);
               worldIn.addEntity(basilemysentity);
            }
         }
      }
...
public boolean isReplaceable(BlockState state, BlockItemUseContext useContext) {
   ...
   private boolean canTrample(World worldIn, Entity trampler) {
      if (trampler instanceof BasilemysEntity) {
         return false;
      } else {
