...
public class PFWorldTypeLayerUtil {
	...
	public static <T extends IArea, C extends IExtendedNoiseRandom<T>> IAreaFactory<T> makeLayers(LongFunction<C> contextFactory, Registry<Biome> registry) {
		...
		IAreaFactory<T> biomes = new PFWorldTypeBiomesLayer().apply(contextFactory.apply(1L));
		...
		IAreaFactory<T> riverLayer = PFWorldTypeRiverLayer.INSTANCE.apply(contextFactory.apply(1L), biomes);
		riverLayer = SmoothLayer.INSTANCE.apply(contextFactory.apply(7000L), riverLayer);
		biomes = PFWorldTypeRiverMixLayer.INSTANCE.apply(contextFactory.apply(100L), biomes, riverLayer);
