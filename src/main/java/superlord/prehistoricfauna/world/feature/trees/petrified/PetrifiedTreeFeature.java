package superlord.prehistoricfauna.world.feature.trees.petrified;
...
public class PetrifiedTreeFeature extends Feature<JohnstoniaConfig>
{
	...
	public boolean generate(ISeedReader worldIn, ChunkGenerator generator, Random rand, BlockPos startPosition, JohnstoniaConfig config) {
		...
		if (pos.getY() >= 1 && pos.getY() + height + 2 <= 256)
		{
			...
			{
				if (isAir(worldIn, new BlockPos(x, j, z))) setBlockState(worldIn, new BlockPos(x, j, z), PFBlocks.PETRIFIED_WOOD.getDefaultState());
			}
			return true;
		}
		return false;
