...
public abstract class PHFAbstractTreeFeature<TFC extends PHFTreeConfig> extends Feature<TFC> {
	...
	public void setDisk(IWorld world, Random random, BlockPos pos, PHFTreeConfig config) {
        ...
        Mutable mutable = new Mutable().setPos(pos);
    ...
    public boolean placeTree(IWorld worldIn, Random rand, BlockPos pos, TFC config) {
        ...
        if (mutableboundingbox.minX > mutableboundingbox.maxX) {
            ...
            try (BlockPos.PooledMutable blockPosPool = BlockPos.PooledMutable.retain()) {
                if (flag && !set.isEmpty()) {
    ...
    protected abstract boolean generate(Set<BlockPos> changedBlocks, IWorld worldIn, Random rand, BlockPos pos, MutableBoundingBox boundsIn, boolean isSapling, TFC config);
