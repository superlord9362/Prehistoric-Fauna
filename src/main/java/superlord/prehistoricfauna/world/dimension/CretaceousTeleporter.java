...
public class CretaceousTeleporter implements ITeleporter {
    ...
    public CretaceousTeleporter(ServerWorld world) {
    	this.world = world;
    }
    ...
    public Optional<TeleportationRepositioner.Result> getExistingPortal(BlockPos pos) {
    	...
    	poiType == PFDimensions.CRETACEOUS_PORTAL.get(), pos, i, PointOfInterestManager.Status.ANY)
    ...
    public Optional<TeleportationRepositioner.Result> makePortal(BlockPos pos, Direction.Axis axis) {
        ...
        BlockState portal = PFBlocks.CRETACEOUS_PORTAL.get().getDefaultState().with(CretaceousPortalBlock.AXIS, axis);
    ...
    public PortalInfo getPortalInfo(Entity entity, ServerWorld destWorld, Function<ServerWorld, PortalInfo> defaultPortalInfo) {
        boolean toCretaceous = destWorld.getDimensionKey() == PFDimensions.CRETACEOUS_WORLD;
        if (entity.world.getDimensionKey() != PFDimensions.CRETACEOUS_WORLD && !toCretaceous) {
            return null;
        } else {
    ...
    private Optional<TeleportationRepositioner.Result> getPortalLogic(Entity entity, BlockPos pos) {
        ...
        if (entity instanceof ServerPlayerEntity) { //ServerPlayerEntity seems to do the portal creation
            if (existing.isPresent()) {
                ...
                Direction.Axis axis = entity.world.getBlockState(entity.field_242271_ac).func_235903_d_(CretaceousPortalBlock.AXIS).orElse(Direction.Axis.X);
