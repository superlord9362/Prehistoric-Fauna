...
public class JurassicTeleporter implements ITeleporter {
    ...
    public JurassicTeleporter(ServerWorld world) {
    	this.world = world;
    }
    ...
    public Optional<TeleportationRepositioner.Result> getExistingPortal(BlockPos pos) {
    	...
    	poiType == PFDimensions.JURASSIC_PORTAL.get(), pos, i, PointOfInterestManager.Status.ANY)
    ...
    public Optional<TeleportationRepositioner.Result> makePortal(BlockPos pos, Direction.Axis axis) {
        ...
        BlockState portal = PFBlocks.JURASSIC_PORTAL.get().getDefaultState().with(JurassicPortalBlock.AXIS, axis);
    ...
    public PortalInfo getPortalInfo(Entity entity, ServerWorld destWorld, Function<ServerWorld, PortalInfo> defaultPortalInfo) {
        boolean toJurassic = destWorld.getDimensionKey() == PFDimensions.JURASSIC_WORLD;
        if (entity.world.getDimensionKey() != PFDimensions.JURASSIC_WORLD && !toJurassic) {
            return null;
        } else {
    ...
    private Optional<TeleportationRepositioner.Result> getPortalLogic(Entity entity, BlockPos pos) {
        ...
        if (entity instanceof ServerPlayerEntity) { //ServerPlayerEntity seems to do the portal creation
            if (existing.isPresent()) {
                ...
                Direction.Axis axis = entity.world.getBlockState(entity.field_242271_ac).func_235903_d_(JurassicPortalBlock.AXIS).orElse(Direction.Axis.X);
