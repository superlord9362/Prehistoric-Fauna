...
public class EntityGallimimusEgg extends EntityThrowable {
    public EntityGallimimusEgg(World worldIn) {
        super(worldIn);
    }

    public EntityGallimimusEgg(World worldIn, EntityLivingBase throwerIn) {
        super(worldIn, throwerIn);
    }

    public EntityGallimimusEgg(World worldIn, double x, double y, double z) {
        super(worldIn, x, y, z);
    }
    ...
    protected void onImpact(RayTraceResult result) {
        ...
        if (!this.world.isRemote) {
            if (this.rand.nextInt(8) == 0) {
                ...
                for (int j = 0; j < i; ++j) {
                    EntityGallimimus entitychicken = new EntityGallimimus(this.world);
